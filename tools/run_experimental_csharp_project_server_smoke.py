...
DEFERRED_INTERACTIVE_HTML_BYTE_LIMIT = 165_500
...
def run(snapshot: Path, output: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="p9.15-server-smoke-") as temporary:
        ...
        try:
            ...
            probes.append({"id": "serves-512x-luminous-alloy-and-continuous-edges", "passed": all(marker in html for marker in (b"rendererMaximumZoom=512", b"effectiveGeometryZoom", b"virtualGeometryScale", b"selectedEdgeRenderedOpacity", b"selectedSourceRenderedWidth", b"edgeNodeAttachmentGap", b"selectedEdgeLinePixelEvidence", b"selectedEdgeLinePixels", b"culledOrdinaryCode", b"node-material-layer", b"materialSprite", b"materialCandidates", b"materialNodePixelEvidence", b"cached-luminous-nebula-alloy-v9", b"zoomAnchorModelPosition", b"intentGraphRuntimeProbe", b"intentgraph-runtime-probe-report", b"graphPixelEvidence", b"scheduleMaterialLayer"))})
            ...
            probes.append({"id": "projects-512x-rendering-contract", "passed": initial_projection["uiContract"]["rendererMaximumZoom"] == 512 and initial_projection["uiContract"]["actualCameraMaximumZoom"] is True and initial_projection["uiContract"]["effectiveGeometryMaximumZoom"] == 512 and initial_projection["uiContract"]["virtualGeometryScaleAtMaximumZoom"] == 1.0 and initial_projection["uiContract"]["selectedEdgeRenderedWidthPixelsAtMaximumZoom"] == 0.65 and initial_projection["uiContract"]["selectedEdgeRenderedOpacityAtMaximumZoom"] == 0.30 and initial_projection["uiContract"]["selectedEdgeEndpointMaterialScaleAtMaximumZoom"] == 0.72 and initial_projection["uiContract"]["selectedEdgeEndpointOpaqueBoundsMaximumPixels"] == 22 and initial_projection["uiContract"]["precisionNodeAttachmentGeometryScale"] == 0.46 and initial_projection["uiContract"]["overviewCodeMaterialMinimumPixels"] == 3.8 and initial_projection["uiContract"]["overviewCodeMaterialOpacity"] == 0.96 and initial_projection["uiContract"]["overviewCodeBaseOpacity"] == 0.30 and initial_projection["uiContract"]["overviewOrdinaryCodeOpacity"] == 0.82 and initial_projection["uiContract"]["overviewCodeRelationOpacity"] == 0.30 and initial_projection["uiContract"]["nebulaBlackMetalNodeMaterial"] is True and initial_projection["uiContract"]["luminousAlloyHierarchyContrast"] is True and initial_projection["uiContract"]["stellarVitreousNodeMaterial"] is False and initial_projection["uiContract"]["celestialCeramicNodeMaterial"] is False and initial_projection["uiContract"]["astralForgedGlassNodeMaterial"] is False and initial_projection["uiContract"]["spectralTitaniumNodeMaterial"] is False and initial_projection["uiContract"]["virtualPrecisionZoom"] is False and initial_projection["uiContract"]["browserRuntimeProbe"] is True and initial_projection["uiContract"]["headlessBrowserRegression"] is True and initial_projection["uiContract"]["canvasPixelEvidence"] is True and initial_projection["uiContract"]["cachedCanvasNodeMaterial"] is True and initial_projection["uiContract"]["boundedMaterialSpriteCache"] is True and initial_projection["uiContract"]["viewportLocalMaterialRendering"] is True and initial_projection["uiContract"]["viewportSpatialMaterialIndex"] is True and initial_projection["uiContract"]["farZoomMaterialCulling"] is True and initial_projection["uiContract"]["selectedMaterialPixelEvidence"] is True})
