...
def build_projection(project_workspace: Path) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    projection = {
        ...
        "uiContract": {
            ...
            "maximumZoom": 100,
            "selectedEdgeRenderedWidthPixels": 0.42,
...
<body>
  ...
  <script>
    ...
    const state = { mode:model.graph.defaultView?.id || 'all', selected:null, selectedWorkId:workItems.length?workItems[workItems.length-1].id:null, stageFocus:null, cy:null, renderTimer:null, workFilterTimer:null, zoomSettleTimer:null, precisionPanTimer:null, detailLevel:null, viewportScale:null, positions:null, visibleNodeIds:null, visibleEdgeIds:null, highlighted:null, highlightedEdgeIds:new Set(), emphasizedNodeIds:new Set(), emphasizedEdgeIds:new Set(), stageAddedNodeIds:new Set(), stageChangedNodeIds:new Set(), stageEdgeIds:new Set(), searchMatchNodeIds:new Set(), onDemandLabelIds:new Set(), precisionStyledNodeIds:new Set(), precisionStyledEdgeIds:new Set(), resizeQueued:false, graphInstanceCount:0, visibilityUpdates:0, renderedWorkCardCount:0 };
    ...
    function makeSpectralObsidianNodeMaterialTexture(){
      const canvas=document.createElement('canvas');canvas.width=canvas.height=256;const context=canvas.getContext('2d');
      context.fillStyle='#01040a';context.fillRect(0,0,256,256);
      const body=context.createRadialGradient(94,76,3,132,135,184);body.addColorStop(0,'rgba(230,247,252,.19)');body.addColorStop(.08,'rgba(116,149,165,.085)');body.addColorStop(.28,'rgba(19,32,47,.18)');body.addColorStop(.62,'rgba(4,10,20,.78)');body.addColorStop(.84,'rgba(1,4,10,.97)');body.addColorStop(1,'rgba(0,1,5,1)');context.fillStyle=body;context.fillRect(0,0,256,256);
      const refractive=context.createLinearGradient(12,238,244,18);refractive.addColorStop(0,'rgba(20,180,183,0)');refractive.addColorStop(.31,'rgba(25,170,177,.032)');refractive.addColorStop(.47,'rgba(226,248,251,.13)');refractive.addColorStop(.52,'rgba(98,190,191,.038)');refractive.addColorStop(.64,'rgba(160,78,137,.035)');refractive.addColorStop(1,'rgba(160,78,137,0)');context.fillStyle=refractive;context.fillRect(0,0,256,256);
      const edgeDepth=context.createRadialGradient(128,128,86,128,128,184);edgeDepth.addColorStop(0,'rgba(0,0,0,0)');edgeDepth.addColorStop(.48,'rgba(0,0,0,.08)');edgeDepth.addColorStop(.73,'rgba(3,12,23,.38)');edgeDepth.addColorStop(.88,'rgba(42,129,141,.075)');edgeDepth.addColorStop(.94,'rgba(184,232,235,.07)');edgeDepth.addColorStop(1,'rgba(0,1,5,.98)');context.fillStyle=edgeDepth;context.fillRect(0,0,256,256);
      const lowerDepth=context.createRadialGradient(178,190,10,164,170,126);lowerDepth.addColorStop(0,'rgba(75,36,80,.05)');lowerDepth.addColorStop(.52,'rgba(7,16,29,.045)');lowerDepth.addColorStop(1,'rgba(0,0,0,0)');context.fillStyle=lowerDepth;context.fillRect(0,0,256,256);
      let seed=9173;for(let index=0;index<154;index+=1){seed=(seed*48271)%2147483647;const x=12+(seed%232);seed=(seed*48271)%2147483647;const y=12+(seed%232);seed=(seed*48271)%2147483647;const alpha=.004+(seed%11)/2400;context.fillStyle=`rgba(215,239,244,${alpha})`;context.fillRect(x,y,1,1);}
      return canvas.toDataURL('image/png');
    }
    const spectralObsidianNodeMaterialTexture=makeSpectralObsidianNodeMaterialTexture();
    function graphStyle(){return [
      {selector:'node',style:{'shape':'ellipse','background-color':'data(color)','background-opacity':.92,'background-blacken':.78,'background-image':spectralObsidianNodeMaterialTexture,'background-image-opacity':.68,'background-fit':'cover','label':'data(label)','color':'#f0f7ff','font-family':'Inter,Segoe UI,Arial,sans-serif','font-weight':600,'font-size':9.5,'text-wrap':'ellipsis','text-max-width':126,'text-valign':'bottom','text-margin-y':5,'width':15,'height':15,'border-width':.65,'border-color':'data(color)','border-opacity':.76,'underlay-color':'data(color)','underlay-opacity':.018,'underlay-padding':1.2,'underlay-shape':'ellipse','overlay-opacity':0,'text-outline-width':1.5,'text-outline-color':'#050812','text-outline-opacity':.94}},
      {selector:'node[category = "code"]',style:{'label':'','background-color':'data(color)','background-opacity':.9,'background-blacken':.68,'background-image':'none','width':2.6,'height':2.6,'opacity':1,'border-width':.3,'border-color':'data(color)','border-opacity':.64,'underlay-color':'data(color)','underlay-opacity':.008,'underlay-padding':.45}},
      {selector:'node[category = "code"][kind = "file"]',style:{'shape':'round-rectangle','width':6.5,'height':6.5,'background-opacity':.98,'background-blacken':.68,'background-image':spectralObsidianNodeMaterialTexture,'background-image-opacity':.74,'border-width':.48,'border-color':'data(color)','border-opacity':.9,'underlay-color':'data(color)','underlay-opacity':.022,'underlay-padding':1.05}},
      {selector:'node[category = "code"][kind = "namespace"]',style:{'shape':'diamond','width':4.3,'height':4.3,'background-image':spectralObsidianNodeMaterialTexture,'background-image-opacity':.55,'border-opacity':.76}},
      {selector:'node[category = "code"][kind = "type"]',style:{'shape':'round-hexagon','width':5.2,'height':5.2,'background-image':spectralObsidianNodeMaterialTexture,'background-image-opacity':.62,'border-opacity':.82}},
      ...
      {selector:'node[category = "code-capsule"]',style:{'background-color':'data(color)','background-opacity':.94,'background-blacken':.82,'background-image':spectralObsidianNodeMaterialTexture,'background-image-opacity':.72,'shape':'round-hexagon','border-width':.7,'border-color':'data(color)','border-opacity':.84,'underlay-color':'data(color)','underlay-opacity':.018,'underlay-padding':1.15,'font-size':9,'font-weight':600,'text-max-width':135,'width':23,'height':23,'color':'#e4f3ff'}},
      ...
      {selector:'node[category = "goal"]',style:{'shape':'round-hexagon'}},
      {selector:'node[category = "intent"]',style:{'shape':'round-hexagon'}},
      ...
      {selector:'node:selected',style:{'border-width':1.15,'border-color':'#d8fbff','border-opacity':.96,'underlay-color':'#61d9e4','underlay-opacity':.055,'underlay-padding':1.45,'overlay-opacity':0,'z-index':99}},
      {selector:'edge:selected',style:{'line-color':'#b9e8ed','opacity':.86,'overlay-opacity':0,'underlay-opacity':0,'z-index':99}},
      {selector:'edge.selection-neighbor',style:{'line-color':'#79bbc2','width':.45,'opacity':.56,'overlay-opacity':0,'underlay-opacity':0,'z-index':98}}
    ];}
    ...
    function renderGraph(options={}) {
      ...
      if(!state.cy){
        ...
        state.cy=cytoscape({
          ...
          minZoom:.10,maxZoom:100,pixelRatio:1,textureOnViewport:false,motionBlur:false,hideEdgesOnViewport:true,boxSelectionEnabled:false,autoungrabify:true
        });
        ...
        state.cy.on('zoom',()=>{updateZoomReadout();updateViewportScale();scheduleSettledSemanticZoom();});
        state.cy.on('pan',()=>{if(!state.viewportScale?.startsWith('precision-'))return;window.clearTimeout(state.precisionPanTimer);state.precisionPanTimer=window.setTimeout(()=>{state.viewportScale=null;updateViewportScale();},90);});
      }
    ...
    function updateZoomReadout(){if(!state.cy)return;const zoom=state.cy.zoom(),display=zoom<1?zoom.toFixed(2):zoom<10?zoom.toFixed(1):Math.round(zoom);document.getElementById('zoomReadout').textContent='x'+display;}
    ...
    function refreshHighlightedEdgeScale(){if(!state.cy||!state.highlightedEdgeIds.size)return;const zoom=Math.max(.1,state.cy.zoom()),readout=document.getElementById('zoomReadout');state.highlightedEdgeIds.forEach(id=>{const selected=state.selected?.type==='edge'&&state.selected.id===id;state.cy.$id(id).style('width',(selected ? 0.42 : 0.27)/zoom);});if(state.selected?.type==='edge'){const edge=allEdges.find(candidate=>candidate.id===state.selected.id),item=state.cy.$id(state.selected.id);readout.dataset.selectedEdgeRenderedWidth=parseFloat(item.renderedStyle('width')).toFixed(3);if(edge){const source=state.cy.$id(edge.source),target=state.cy.$id(edge.target),sourceBounds=source.renderedBoundingBox({includeLabels:false,includeOverlays:false,includeUnderlays:false}),targetBounds=target.renderedBoundingBox({includeLabels:false,includeOverlays:false,includeUnderlays:false});readout.dataset.selectedSourceRenderedWidth=parseFloat(source.renderedStyle('width')).toFixed(3);readout.dataset.selectedTargetRenderedWidth=parseFloat(target.renderedStyle('width')).toFixed(3);readout.dataset.selectedSourceRenderedBorder=parseFloat(source.renderedStyle('border-width')).toFixed(3);readout.dataset.selectedTargetRenderedBorder=parseFloat(target.renderedStyle('border-width')).toFixed(3);readout.dataset.selectedSourceRenderedUnderlay=parseFloat(source.renderedStyle('underlay-padding')).toFixed(3);readout.dataset.selectedTargetRenderedUnderlay=parseFloat(target.renderedStyle('underlay-padding')).toFixed(3);readout.dataset.selectedSourceRenderedBounds=`${sourceBounds.w.toFixed(3)}x${sourceBounds.h.toFixed(3)}`;readout.dataset.selectedTargetRenderedBounds=`${targetBounds.w.toFixed(3)}x${targetBounds.h.toFixed(3)}`;}}}
    function precisionViewportElements(){
      const extent=state.cy.extent(),marginX=Math.max(1,(extent.x2-extent.x1)*.4),marginY=Math.max(1,(extent.y2-extent.y1)*.4),bounds={x1:extent.x1-marginX,y1:extent.y1-marginY,x2:extent.x2+marginX,y2:extent.y2+marginY},nodeIds=new Set(),edgeIds=new Set();
      state.positions.forEach((position,id)=>{if(position.x>=bounds.x1&&position.x<=bounds.x2&&position.y>=bounds.y1&&position.y<=bounds.y2)nodeIds.add(id);});
      state.highlightedEdgeIds.forEach(id=>{edgeIds.add(id);const edge=allEdges.find(candidate=>candidate.id===id);if(edge){nodeIds.add(edge.source);nodeIds.add(edge.target);}});
      allEdges.forEach(edge=>{const source=state.positions.get(edge.source),target=state.positions.get(edge.target);if(!source||!target)return;const intersects=Math.max(source.x,target.x)>=bounds.x1&&Math.min(source.x,target.x)<=bounds.x2&&Math.max(source.y,target.y)>=bounds.y1&&Math.min(source.y,target.y)<=bounds.y2;if(intersects){edgeIds.add(edge.id);nodeIds.add(edge.source);nodeIds.add(edge.target);}});
      if(state.selected?.type==='node')nodeIds.add(state.selected.id);
      return {nodeIds,edgeIds};
    }
    function updateViewportScale() {
      if(!state.cy)return;
      const zoom=state.cy.zoom(),deepZoomFloors=[18,24,32,45,60,80,100],deepFloor=zoom>=18?deepZoomFloors.reduce((floor,candidate)=>zoom>=candidate?candidate:floor,18):null,band=zoom<.25?'far':zoom<.52?'overview':zoom<1.05?'structure':zoom<1.8?'detail':zoom<3.5?'close':zoom<7?'deep':zoom<18?'inspect':`precision-${deepFloor}`;
      if(state.viewportScale===band){refreshHighlightedEdgeScale();return;}
      state.viewportScale=band;
      const profiles={far:{scale:7,node:18,code:2.6,file:5.8,namespace:3.8,type:4.7,capsule:20,label:9.5,onDemandLabel:8.5,border:.65,codeBorder:.3,glow:1.2,edge:.34,codeEdge:.18,semanticEdge:.84},overview:{scale:3,node:18,code:2.8,file:6.2,namespace:4.1,type:5,capsule:21,label:9.5,onDemandLabel:8.5,border:.65,codeBorder:.3,glow:1.2,edge:.4,codeEdge:.22,semanticEdge:.92},structure:{scale:1.4,node:19,code:3.5,file:7,namespace:4.8,type:5.8,capsule:22,label:9.5,onDemandLabel:8.5,border:.65,codeBorder:.3,glow:1.2,edge:.48,codeEdge:.28,semanticEdge:1},detail:{scale:.75,node:20,code:5,file:8.4,namespace:6,type:7.1,capsule:24,label:9.5,onDemandLabel:8.5,border:.65,codeBorder:.3,glow:1.2,edge:.54,codeEdge:.34,semanticEdge:1.08},close:{scale:.38,node:21,code:8,file:10,namespace:8.4,type:9.2,capsule:26,label:9.5,onDemandLabel:8.5,border:.65,codeBorder:.3,glow:1.2,edge:.62,codeEdge:.46,semanticEdge:1.16},deep:{scale:.19,node:22,code:12,file:14,namespace:12.5,type:13.2,capsule:28,label:9.5,onDemandLabel:8.5,border:.65,codeBorder:.3,glow:1.2,edge:.7,codeEdge:.62,semanticEdge:1.24},inspect:{scale:.085,node:23,code:17,file:20,namespace:18,type:19,capsule:30,label:9.5,onDemandLabel:8.5,border:.65,codeBorder:.3,glow:1.2,edge:.78,codeEdge:.78,semanticEdge:1.34}};
      const precisionProfile={scale:deepFloor?14/(23*deepFloor):1,node:26,code:23,file:26,namespace:24,type:25,capsule:35,label:13.5,onDemandLabel:12.5,border:.65,codeBorder:.3,glow:1.2,edge:1.02,codeEdge:1.02,semanticEdge:1.55};
      const precision=band.startsWith('precision-'),profile=precision?precisionProfile:profiles[band],size=value=>Math.max(.001,Math.round(value*profile.scale*1000)/1000),dynamicStyles=[
        {selector:'node',style:{'width':size(profile.node),'height':size(profile.node),'font-size':size(profile.label),'border-width':size(profile.border),'underlay-padding':size(profile.glow),'text-margin-y':size(5),'text-outline-width':size(1.5)}},
        {selector:'node.show-on-demand-label',style:{'font-size':size(profile.onDemandLabel)}},
        {selector:'node[category = "code"]',style:{'width':size(profile.code),'height':size(profile.code),'border-width':size(profile.codeBorder),'underlay-padding':size(.45)}},
        {selector:'node[category = "code"][kind = "file"]',style:{'width':size(profile.file),'height':size(profile.file),'border-width':size(profile.codeBorder*1.6),'underlay-padding':size(1.05),'background-image':spectralObsidianNodeMaterialTexture,'background-image-opacity':.74}},
      ...
      if(precision)dynamicStyles.push({selector:'node',style:{'underlay-opacity':0}});
      if(!precision){
        state.cy.batch(()=>{state.precisionStyledNodeIds.forEach(id=>state.cy.$id(id).removeStyle('width height font-size border-width underlay-padding text-margin-y text-outline-width'));state.precisionStyledEdgeIds.forEach(id=>state.cy.$id(id).removeStyle('width'));});
        state.precisionStyledNodeIds=new Set();state.precisionStyledEdgeIds=new Set();
        state.cy.style().fromJson(graphStyle().concat(dynamicStyles)).update();
      }else{
        const viewport=precisionViewportElements(),targetNodes=state.cy.nodes().filter(node=>viewport.nodeIds.has(node.id())),targetEdges=state.cy.edges().filter(edge=>viewport.edgeIds.has(edge.id())),apply=(selector,style)=>{const target=selector.startsWith('node')?targetNodes:targetEdges;(selector==='node'||selector==='edge'?target:target.filter(selector)).style(style);};
        state.cy.batch(()=>dynamicStyles.forEach(item=>apply(item.selector,item.style)));
        viewport.nodeIds.forEach(id=>state.precisionStyledNodeIds.add(id));viewport.edgeIds.forEach(id=>state.precisionStyledEdgeIds.add(id));
        const readout=document.getElementById('zoomReadout');readout.dataset.precisionNodeCount=String(targetNodes.length);readout.dataset.precisionEdgeCount=String(targetEdges.length);
      }
    ...
    document.getElementById('previousWork').addEventListener('click',()=>moveWork(-1));document.getElementById('nextWork').addEventListener('click',()=>moveWork(1));document.getElementById('previousStage').addEventListener('click',()=>moveStage(-1));document.getElementById('nextStage').addEventListener('click',()=>moveStage(1));document.getElementById('zoomIn').addEventListener('click',()=>state.cy.zoom({level:state.cy.zoom()*1.8,renderedPosition:zoomAnchorPosition()}));document.getElementById('zoomOut').addEventListener('click',()=>state.cy.zoom({level:state.cy.zoom()/1.8,renderedPosition:zoomAnchorPosition()}));document.getElementById('zoom100').addEventListener('click',()=>state.cy.zoom({level:100,renderedPosition:zoomAnchorPosition()}));document.getElementById('fitGraph').addEventListener('click',()=>{const graph=visibleGraphData(),semanticFocus=state.mode==='overview'&&!filters().search&&!filters().category&&!filters().relation?model.graph.views.overview.nodeIds.map(id=>nodeById.get(id)).filter(Boolean):graph.nodes;fitNodes(semanticFocus);updateZoomReadout();});init();staticPanels();renderGraph({fit:true});updateZoomReadout();renderSelection();resize();window.dispatchEvent(new Event('intentgraph-ready'));
    }
...
def validate_output(output: Path, project_workspace: Path, expected_projection: dict[str, Any], project_before: list[dict[str, str]]) -> dict[str, Any]:
    ...
    for marker in ["id=\"projectGraph\"", "id=\"modeBadge\"", "id=\"workList\"", "id=\"previousWork\"", "id=\"nextWork\"", "id=\"workPosition\"", "id=\"stageList\"", "id=\"previousStage\"", "id=\"nextStage\"", "id=\"stagePosition\"", "id=\"selectionInspector\"", "id=\"changePanel\"", "id=\"deltaList\"", "id=\"evidencePanel\"", "id=\"authorityPanel\"", "id=\"zoomIn\"", "id=\"zoomOut\"", "id=\"zoom100\"", "id=\"zoomReadout\"", "maxZoom:100", "deepZoomFloors", "precision-", "makeSpectralObsidianNodeMaterialTexture", "edge:selected", "assets/cytoscape.min.js"]:
