...
def build_projection(project_workspace: Path) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    projection = {
        ...
        "uiContract": {
            ...
            "selectedEdgeRenderedWidthPixelsAtMaximumZoom": 0.65,
            "selectedEdgeRenderedOpacityAtMaximumZoom": 0.30,
            "selectedEdgeEndpointMaterialScaleAtMaximumZoom": 0.72,
            "selectedEdgeEndpointOpaqueBoundsMaximumPixels": 22,
            "precisionNodeAttachmentGeometryScale": 0.46,
            "overviewCodeMaterialMinimumPixels": 3.8,
            "overviewCodeMaterialOpacity": 0.96,
            "overviewCodeBaseOpacity": 0.30,
            "overviewOrdinaryCodeOpacity": 0.82,
            "overviewCodeRelationOpacity": 0.30,
            ...
            "nebulaBlackMetalNodeMaterial": True,
            "luminousAlloyHierarchyContrast": True,
            ...
            "boundedMaterialSpriteCache": True,
            "viewportLocalMaterialRendering": True,
            "viewportSpatialMaterialIndex": True,
            "farZoomMaterialCulling": True,
            "selectedMaterialPixelEvidence": True,
...
<body>
  ...
  <script>
    ...
    const colors = { code:'#63b8d0', project:'#67d4d0', 'source-document':'#85a0ba', goal:'#edbd68', capability:'#6ec59c', constraint:'#df8569', 'verification-requirement':'#a38bc4', work:'#71c39e', intent:'#d17fa9', mapping:'#d0ad61', proposal:'#db6f7c', verification:'#9084bd', evidence:'#a083c8', 'review-receipt':'#ab82cd', 'verifier-result':'#70d2bd', 'evidence-decision':'#ddb75f', authority:'#79a2d1', history:'#8094c4' };
    const communityPalette = ['#53cde9','#d7b252','#d06f88','#56c9be','#79c98d','#cfa358','#a98bd9','#d284b2','#c6885b','#7fa9dd'];
    ...
    function graphStyle(){return [
      ...
      {selector:'node[category = "code"]',style:{'label':'','background-fill':'solid','background-color':'data(color)','background-opacity':.30,'background-blacken':.24,'width':2.2,'height':2.2,'opacity':1,'border-width':.12,'border-color':'#d7f8fb','border-opacity':.22,'outline-width':0,'outline-opacity':0,'underlay-opacity':0,'underlay-padding':0}},
      {selector:'node[category = "code"][kind = "file"]',style:{...spectralObsidianOpticalMaterial,'shape':'round-rectangle','width':6.5,'height':6.5,'background-color':'data(color)','background-opacity':.22,'background-blacken':.22,'border-width':.42,'border-color':'#d8fbff','border-opacity':.42,'underlay-opacity':0,'underlay-padding':0}},
      ...
      {selector:'node[category = "code-capsule"]',style:{...spectralObsidianOpticalMaterial,'background-color':'data(color)','background-opacity':.20,'background-blacken':.22,'shape':'round-hexagon','border-width':.46,'border-opacity':.38,'underlay-opacity':0,'underlay-padding':0,'font-size':9,'font-weight':600,'text-max-width':135,'width':19,'height':19,'color':'#dcebf3'}},
      ...
      {selector:'node[category = "code"].low-detail',style:{'opacity':.82}},
      ...
      {selector:'edge[category = "code-relation"]',style:{'width':.28,'line-color':'data(color)','opacity':.30}},
      {selector:'edge.low-detail',style:{'opacity':.09}},
      ...
      {selector:'edge:selected',style:{'line-color':'#a8d8dc','line-style':'solid','opacity':.46,'overlay-opacity':0,'underlay-opacity':0,'z-index':99}},
    ...
    function highlightedEdgeScreenWidth(zoom,selected){if(zoom>=384)return selected ? .65 : .50;if(zoom>=256)return selected ? .68 : .52;if(zoom>=160)return selected ? .72 : .54;if(zoom>=100)return selected ? .78 : .56;if(zoom>=80)return selected ? .84 : .58;if(zoom>=45)return selected ? .92 : .62;if(zoom>=18)return selected ? 1.02 : .68;return selected ? 1.18 : .78;}
    function highlightedEdgeOpacity(zoom,selected){if(zoom>=384)return selected ? .30 : .10;if(zoom>=256)return selected ? .32 : .11;if(zoom>=160)return selected ? .34 : .12;if(zoom>=100)return selected ? .36 : .13;if(zoom>=80)return selected ? .38 : .14;if(zoom>=45)return selected ? .40 : .15;if(zoom>=18)return selected ? .43 : .17;return selected ? .48 : .20;}
    ...
    function updateViewportScale() {
      ...
      state.viewportScale=band;document.getElementById('projectGraph').dataset.scaleBand=band;
      ...
      const precision=band.startsWith('precision-'),profile=precision?precisionProfile:profiles[band],geometryScale=precision?.46:1,size=value=>Math.max(.001,Math.round(value*profile.scale*1000)/1000),dynamicStyles=[
        {selector:'node',style:{'width':size(profile.node*geometryScale),'height':size(profile.node*geometryScale),'font-size':size(profile.label),'text-max-width':size(profile.labelWidth),'border-width':size(profile.border),'outline-width':size(profile.border*.48),'outline-offset':size(profile.border*.65),'underlay-padding':size(profile.glow),'text-margin-y':size(5),'text-outline-width':size(1.35)}},
        {selector:'node.show-on-demand-label',style:{'font-size':size(profile.onDemandLabel),'text-max-width':size(profile.labelWidth)}},
        {selector:'node[category = "code"]',style:{'width':size(profile.code*geometryScale),'height':size(profile.code*geometryScale),'border-width':size(profile.codeBorder),'outline-width':0,'outline-opacity':0,'underlay-padding':size(.45)}},
        {selector:'node[category = "code"][kind = "file"]',style:{'width':size(profile.file*geometryScale),'height':size(profile.file*geometryScale),'border-width':size(profile.codeBorder*1.6),'outline-width':size(profile.border*.42),'outline-opacity':.18,'outline-offset':size(profile.border*.55),'underlay-padding':size(1.05)}},
        {selector:'node[category = "code"][kind = "namespace"]',style:{'width':size(profile.namespace*geometryScale),'height':size(profile.namespace*geometryScale)}},
        {selector:'node[category = "code"][kind = "type"]',style:{'width':size(profile.type*geometryScale),'height':size(profile.type*geometryScale)}},
        {selector:'node[category = "code-capsule"]',style:{'width':size(profile.capsule*geometryScale),'height':size(profile.capsule*geometryScale),'border-width':size(profile.border),'underlay-padding':size(1.15)}},
    ...
    function selectedEdgeLinePixelEvidence(edge,endpointPixels){const empty={sampleCount:0,observedSampleCount:0,longestMissingRun:0};try{const source=state.cy.$id(edge.source),target=state.cy.$id(edge.target);if(!source.length||!target.length||endpointPixels.opaqueSampleCount<=0)return empty;const start=source.renderedPosition(),end=target.renderedPosition(),length=Math.hypot(end.x-start.x,end.y-start.y);if(!length)return empty;const direction={x:(end.x-start.x)/length,y:(end.y-start.y)/length},bodyRadius=Math.max(endpointPixels.opaqueBoundsWidth,endpointPixels.opaqueBoundsHeight)/2,canvases=[...document.querySelectorAll('#projectGraph canvas')].filter(canvas=>!String(canvas.className).includes('node-material-layer'));let observedSampleCount=0,longestMissingRun=0,missingRun=0,sampleCount=0;for(let step=0;step<=12;step+=1){const x=start.x+direction.x*(bodyRadius+step),y=start.y+direction.y*(bodyRadius+step);if(x<0||x>=state.cy.width()||y<0||y>=state.cy.height())continue;sampleCount+=1;const observed=canvases.some(canvas=>{const ratio=canvas.width/Math.max(1,state.cy.width()),cx=Math.round(x*ratio),cy=Math.round(y*ratio),radius=Math.max(1,Math.round(1.5*ratio)),left=Math.max(0,cx-radius),top=Math.max(0,cy-radius),right=Math.min(canvas.width,cx+radius+1),bottom=Math.min(canvas.height,cy+radius+1);if(right<=left||bottom<=top)return false;const pixels=canvas.getContext('2d',{willReadFrequently:true}).getImageData(left,top,right-left,bottom-top).data;for(let offset=3;offset<pixels.length;offset+=4)if(pixels[offset]>4)return true;return false;});if(observed){observedSampleCount+=1;missingRun=0;}else{missingRun+=1;longestMissingRun=Math.max(longestMissingRun,missingRun);}}return {sampleCount,observedSampleCount,longestMissingRun};}catch(error){runtimeProbeErrors.push('selected edge pixel sample: '+String(error.message||error));return empty;}}
    function graphPixelEvidence(){const canvases=[...document.querySelectorAll('#projectGraph canvas')],layers=canvases.map(canvas=>({className:canvas.className||'',...sampledCanvasPixels(canvas)})),material=layers.find(layer=>String(layer.className).includes('node-material-layer'))||null,ordinaryCodeNode=allNodes.find(node=>node.category==='code'&&!['file','namespace','type'].includes(node.kind)&&!importantCodeLabelIds.has(node.id)),ordinaryCodeItem=ordinaryCodeNode?state.cy?.$id(ordinaryCodeNode.id):null,ordinaryCodeVisibility=ordinaryCodeItem?.length?{nodeId:ordinaryCodeNode.id,opacity:Number(ordinaryCodeItem.style('opacity')),backgroundOpacity:Number(ordinaryCodeItem.style('background-opacity')),renderedWidth:Number.parseFloat(ordinaryCodeItem.renderedStyle('width'))}:{nodeId:'',opacity:0,backgroundOpacity:0,renderedWidth:0};return {canvasCount:layers.length,totalOpaqueSampleCount:layers.reduce((total,layer)=>total+layer.opaqueSampleCount,0),totalChromaticSampleCount:layers.reduce((total,layer)=>total+layer.chromaticSampleCount,0),materialOpaqueSampleCount:material?.opaqueSampleCount||0,materialChromaticSampleCount:material?.chromaticSampleCount||0,materialMeanOpaqueLuminance:material?.meanOpaqueLuminance||0,ordinaryCodeVisibility,layers};}
    function runtimeProbeState(){const readout=document.getElementById('zoomReadout'),material=document.querySelector('.node-material-layer'),selectedRelation=state.selected?.type==='edge'?edgeById.get(state.selected.id):null,selectedEdgeItem=selectedRelation?state.cy.$id(selectedRelation.id):null,sourcePixels=selectedRelation?materialNodePixelEvidence(selectedRelation.source):null;return {logicalZoom:Number(readout.textContent.replace('x','')),rendererZoom:Number(readout.dataset.rendererZoom||0),virtualGeometryScale:Number(readout.dataset.virtualGeometryScale||0),effectiveGeometryZoom:Number(readout.dataset.effectiveGeometryZoom||0),selectedEdgeRenderedWidth:Number(readout.dataset.selectedEdgeRenderedWidth||0),selectedEdgeRenderedOpacity:Number(readout.dataset.selectedEdgeRenderedOpacity||0),selectedEdgeLineStyle:selectedEdgeItem?.length?selectedEdgeItem.style('line-style'):'',selectedSourceRenderedWidth:Number(readout.dataset.selectedSourceRenderedWidth||0),selectedTargetRenderedWidth:Number(readout.dataset.selectedTargetRenderedWidth||0),selectedEdgeLinePixels:selectedRelation?selectedEdgeLinePixelEvidence(selectedRelation,sourcePixels):{sampleCount:0,observedSampleCount:0,longestMissingRun:0},materialProfile:material?.dataset.material||'',materialDrawCount:Number(material?.dataset.drawCount||0),materialCandidateCount:Number(material?.dataset.candidateCount||0),materialOffscreenCulledCount:Number(material?.dataset.offscreenCulled||0),materialCulledOrdinaryCodeCount:Number(material?.dataset.culledOrdinaryCode||0),materialSpriteCount:Number(material?.dataset.spriteCount||0)};}
    ...
    async function runRuntimeProbe(){const pause=milliseconds=>new Promise(resolve=>window.setTimeout(resolve,milliseconds)),frames=()=>new Promise(resolve=>window.requestAnimationFrame(()=>window.requestAnimationFrame(resolve))),endpointDistance=edge=>{const source=state.cy.$id(edge.source),target=state.cy.$id(edge.target);if(!source.length||!target.length)return 0;const a=source.position(),b=target.position();return Math.hypot(a.x-b.x,a.y-b.y);},nearestCenterDistance=()=>{const center={x:state.cy.width()/2,y:state.cy.height()/2};let distance=Infinity;state.cy.nodes(':visible').forEach(item=>{const point=item.renderedPosition();distance=Math.min(distance,Math.hypot(point.x-center.x,point.y-center.y));});return distance;};await pause(450);drawMaterialLayer();await pause(80);const selectedEdge=allEdges.find(edge=>edge.kind==='invokes-syntax')||allEdges.find(edge=>edge.category==='semantic-relation')||allEdges[0];if(!selectedEdge)throw new Error('runtime probe requires at least one relation');const overview={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeEndpointDistance:endpointDistance(selectedEdge)};document.getElementById('zoomMax').click();await frames();await pause(80);const unselectedMaximumAnchorDistance=nearestCenterDistance();document.getElementById('fitGraph').click();await frames();await pause(50);const hiddenAnchorNode=state.cy.nodes(':visible')[0],hiddenAnchorDisplay=hiddenAnchorNode?.style('display');if(hiddenAnchorNode?.length){state.selected={type:'node',id:hiddenAnchorNode.id()};hiddenAnchorNode.style('display','none');}document.getElementById('zoomMax').click();await frames();await pause(80);const hiddenSelectionMaximumAnchorDistance=nearestCenterDistance();if(hiddenAnchorNode?.length)hiddenAnchorNode.style('display',hiddenAnchorDisplay||'element');state.selected=null;document.getElementById('fitGraph').click();await frames();await pause(80);state.selected={type:'edge',id:selectedEdge.id};renderSelection();highlight();const hundredStarted=performance.now();document.getElementById('zoom100').click();await frames();drawMaterialLayer();const hundredInteractionMilliseconds=performance.now()-hundredStarted;await pause(90);updateZoomReadout();const hundred=runtimeProbeState(),maximumStarted=performance.now();document.getElementById('zoomMax').click();await frames();drawMaterialLayer();const maximumInteractionMilliseconds=performance.now()-maximumStarted;await pause(110);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();drawMaterialLayer();await pause(60);const panBefore={...state.cy.pan()},panStarted=performance.now();state.cy.pan({x:panBefore.x+18,y:panBefore.y-12});await frames();drawMaterialLayer();const panInteractionMilliseconds=performance.now()-panStarted,panAfter=state.cy.pan(),panDelta={x:panAfter.x-panBefore.x,y:panAfter.y-panBefore.y};state.cy.pan(panBefore);await frames();document.getElementById('zoomOut').click();await frames();await pause(70);updateZoomReadout();const afterZoomOut=runtimeProbeState();document.getElementById('zoomMax').click();await frames();await pause(100);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();drawMaterialLayer();await pause(60);const selectedSourceMaterialPixels=materialNodePixelEvidence(selectedEdge.source),selectedTargetMaterialPixels=materialNodePixelEvidence(selectedEdge.target),maximum={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeId:selectedEdge.id,selectedEdgeKind:selectedEdge.kind,selectedEdgeEndpointDistance:endpointDistance(selectedEdge),selectedEndpointMaterialPixels:selectedSourceMaterialPixels,selectedSourceEndpointMaterialPixels:selectedSourceMaterialPixels,selectedTargetEndpointMaterialPixels:selectedTargetMaterialPixels,selectionText:document.getElementById('selectionInspector').innerText,navigation:{hundred,afterZoomOut,panDelta,unselectedMaximumAnchorDistance,hiddenSelectionMaximumAnchorDistance,hundredInteractionMilliseconds,maximumInteractionMilliseconds,panInteractionMilliseconds}};maximum.endpointGeometryScale=overview.selectedEdgeEndpointDistance>0?maximum.selectedEdgeEndpointDistance/overview.selectedEdgeEndpointDistance:0;const sampledEndpointGeometry=[[maximum.state.selectedSourceRenderedWidth,selectedSourceMaterialPixels],[maximum.state.selectedTargetRenderedWidth,selectedTargetMaterialPixels]].filter(([,pixels])=>pixels.opaqueSampleCount>0);maximum.sampledSelectedEndpointCount=sampledEndpointGeometry.length;maximum.edgeNodeAttachmentGap=sampledEndpointGeometry.length?Math.max(...sampledEndpointGeometry.map(([renderedWidth,pixels])=>Math.abs(renderedWidth-pixels.opaqueBoundsWidth))):Infinity;const checks=[
      ...
      {id:'overview-material-visible-contrast',passed:overview.pixels.materialOpaqueSampleCount>=400&&overview.pixels.materialChromaticSampleCount>=280&&overview.pixels.materialMeanOpaqueLuminance>=34,actual:overview.pixels,expected:'material opaque>=400, chromatic>=280, mean luminance>=34'},
      {id:'overview-ordinary-code-visible',passed:Boolean(overview.pixels.ordinaryCodeVisibility.nodeId)&&overview.pixels.ordinaryCodeVisibility.opacity>=.82&&overview.pixels.ordinaryCodeVisibility.backgroundOpacity>=.30&&overview.pixels.ordinaryCodeVisibility.renderedWidth>=1.5,actual:overview.pixels.ordinaryCodeVisibility,expected:'ordinary code opacity>=.82, background opacity>=.30, rendered width>=1.5px'},
      {id:'luminous-alloy-material-active',passed:maximum.state.materialProfile==='cached-luminous-nebula-alloy-v9',actual:maximum.state.materialProfile,expected:'cached-luminous-nebula-alloy-v9'},
      {id:'material-sprite-cache-bounded',passed:maximum.state.materialSpriteCount>0&&maximum.state.materialSpriteCount<=materialSpriteCacheLimit,actual:maximum.state.materialSpriteCount,expected:'1..'+materialSpriteCacheLimit},
      {id:'material-viewport-candidates-bounded',passed:maximum.state.materialCandidateCount>0&&maximum.state.materialCandidateCount<=Math.max(256,Math.ceil(allNodes.length*.10)),actual:maximum.state.materialCandidateCount,expected:'1..'+Math.max(256,Math.ceil(allNodes.length*.10))},
      {id:'selected-endpoint-material-detailed',passed:maximum.selectedEndpointMaterialPixels.opaqueSampleCount>=60&&maximum.selectedEndpointMaterialPixels.chromaticSampleCount>=12&&maximum.selectedEndpointMaterialPixels.uniqueColorBucketCount>=8&&maximum.selectedEndpointMaterialPixels.luminanceRange>=35,actual:maximum.selectedEndpointMaterialPixels,expected:'opaque>=60, chromatic>=12, color buckets>=8, luminance range>=35'},
      {id:'selected-endpoint-material-compact',passed:sampledEndpointGeometry.length>0&&sampledEndpointGeometry.every(([,pixels])=>pixels.opaqueBoundsWidth>0&&pixels.opaqueBoundsWidth<=22&&pixels.opaqueBoundsHeight>0&&pixels.opaqueBoundsHeight<=22),actual:{sampledEndpointCount:sampledEndpointGeometry.length,source:maximum.selectedSourceEndpointMaterialPixels,target:maximum.selectedTargetEndpointMaterialPixels},expected:'at least one on-screen endpoint sampled; every sampled opaque bound >0 and <=22x22 screen pixels'},
      {id:'selected-edge-node-attachment-continuous',passed:sampledEndpointGeometry.length>0&&Number.isFinite(maximum.edgeNodeAttachmentGap)&&maximum.edgeNodeAttachmentGap<=1.5,actual:{sampledEndpointCount:sampledEndpointGeometry.length,diameterDifference:maximum.edgeNodeAttachmentGap},expected:'at least one on-screen endpoint sampled; rendered/body diameter difference<=1.5 screen pixels'},
      ...
      {id:'selected-edge-line-pixel-continuous',passed:maximum.state.selectedEdgeLinePixels.sampleCount===13&&maximum.state.selectedEdgeLinePixels.observedSampleCount>=12&&maximum.state.selectedEdgeLinePixels.observedSampleCount<=maximum.state.selectedEdgeLinePixels.sampleCount&&maximum.state.selectedEdgeLinePixels.longestMissingRun<=1,actual:maximum.state.selectedEdgeLinePixels,expected:'sample count=13, 12..13 observed samples, longest missing run<=1'},
      ...
      {id:'selected-edge-screen-width',passed:Math.abs(maximum.state.selectedEdgeRenderedWidth-.65)<.02,actual:maximum.state.selectedEdgeRenderedWidth,expected:.65},
      {id:'selected-edge-opacity',passed:Math.abs(maximum.state.selectedEdgeRenderedOpacity-.30)<.01,actual:maximum.state.selectedEdgeRenderedOpacity,expected:.30},
...
def validate_projection(projection: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(ui_contract, dict) or any(ui_contract.get(key) is not True for key in ("fullGraphDefault", "allNodesLoaded", "allEdgesLoaded", "progressiveDetail", "viewportScaleCompensation", "relationAwareCommunityLayout", "zoomStyleBuckets", "spectralObsidianNodeMaterial", "iridescentVoidNodeMaterial", "nebulaBlackMetalNodeMaterial", "luminousAlloyHierarchyContrast", "actualCameraMaximumZoom", "browserRuntimeProbe", "headlessBrowserRegression", "canvasPixelEvidence", "rendererSafeOpticalMaterial", "cachedCanvasNodeMaterial", "boundedMaterialSpriteCache", "viewportLocalMaterialRendering", "viewportSpatialMaterialIndex", "farZoomMaterialCulling", "selectedMaterialPixelEvidence", "selectedEdgeScreenSpaceTaper", "precisionDeepZoomBands", "supportingNodeLabelsOnDemand", "workStageTimeline", "durableWorkStageRevisions", "allRecordedWorkItemsNavigable", "workHistorySearch", "workHistoryStatusFilter", "boundedWorkHistoryRendering", "previousNextWorkNavigation", "previousNextStageNavigation", "liveProjectionRefreshAfterMutation", "staticRevisionSnapshotImmutable", "loopbackProjectStateMutationFromUi", "loopbackReviewProposalIntakeFromUi", "loopbackGuidedReviewProposalFromUi", "loopbackReviewReceiptIntakeFromUi", "loopbackGuidedReviewReceiptFromUi", "loopbackVerifierResultIntakeFromUi", "loopbackEvidenceDecisionFromUi", "clientSideEvidenceArtifactHashing", "externalEvidenceAcceptanceByWorkbench")) or ui_contract.get("maximumZoom") != 512 or ui_contract.get("rendererMaximumZoom") != 512 or ui_contract.get("effectiveGeometryMaximumZoom") != 512 or ui_contract.get("virtualGeometryScaleAtMaximumZoom") != 1.0 or ui_contract.get("selectedEdgeRenderedWidthPixelsAtMaximumZoom") != 0.65 or ui_contract.get("selectedEdgeRenderedOpacityAtMaximumZoom") != 0.30 or ui_contract.get("selectedEdgeEndpointMaterialScaleAtMaximumZoom") != 0.72 or ui_contract.get("selectedEdgeEndpointOpaqueBoundsMaximumPixels") != 22 or ui_contract.get("precisionNodeAttachmentGeometryScale") != 0.46 or ui_contract.get("overviewCodeMaterialMinimumPixels") != 3.8 or ui_contract.get("overviewCodeMaterialOpacity") != 0.96 or ui_contract.get("overviewCodeBaseOpacity") != 0.30 or ui_contract.get("overviewOrdinaryCodeOpacity") != 0.82 or ui_contract.get("overviewCodeRelationOpacity") != 0.30 or ui_contract.get("evidenceAcceptanceAuthorityScope") != "local-project-workspace" or any(ui_contract.get(key) is not False for key in ("stellarVitreousNodeMaterial", "celestialCeramicNodeMaterial", "spectralTitaniumNodeMaterial", "astralForgedGlassNodeMaterial", "virtualPrecisionZoom", "staticGraphMutationFromUi", "targetRepositoryMutationFromUi", "physicsLayoutOnLoad", "externalVerifierExecutionByWorkbench", "reviewerAuthenticationByWorkbench")):
