...
def build_projection(project_workspace: Path) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    projection = {
        ...
        "uiContract": {
            ...
            "selectedEdgeRenderedWidthPixelsAtMaximumZoom": 0.55,
            "selectedEdgeRenderedOpacityAtMaximumZoom": 0.68,
            ...
            "astralForgedGlassNodeMaterial": True,
...
<body>
  ...
  <script>
    ...
    const precisionZoomPivot=18,rendererMaximumZoom=100,logicalMaximumZoom=100,materialSpriteCacheLimit=96,materialSpatialCellSize=120;
    ...
    function materialSprite(node,selected,accent){const color=nodeColor(node),shape=materialShape(node),key=['astral-forged-glass-v3',color,shape,selected?'selected':'normal',accent||''].join('|');if(state.materialSpriteCache.has(key))return state.materialSpriteCache.get(key);const canvas=document.createElement('canvas');canvas.width=canvas.height=128;const context=canvas.getContext('2d'),cx=64,cy=64,radius=27.5;
      // One cached optical asset per material state keeps the graph responsive while the finish stays crisp at inspection scale.
      const halo=context.createRadialGradient(cx,cy,radius*.72,cx,cy,48);halo.addColorStop(0,rgba(color,selected?.16:.065));halo.addColorStop(.46,rgba(color,selected?.055:.018));halo.addColorStop(1,rgba(color,0));context.fillStyle=halo;context.fillRect(0,0,128,128);
      context.save();context.shadowColor='rgba(0,0,0,.95)';context.shadowBlur=8;context.shadowOffsetY=3;traceMaterialShape(context,shape,cx,cy,radius+1.4);context.fillStyle='rgba(0,2,7,.98)';context.fill();context.restore();
      context.save();traceMaterialShape(context,shape,cx,cy,radius);context.clip();context.fillStyle='#02050b';context.fillRect(32,32,64,64);
      const forged=context.createLinearGradient(39,94,91,34);forged.addColorStop(0,'rgba(0,2,7,1)');forged.addColorStop(.30,'rgba(4,9,16,1)');forged.addColorStop(.445,rgba(color,.105));forged.addColorStop(.49,'rgba(194,231,234,.19)');forged.addColorStop(.515,rgba(color,.075));forged.addColorStop(.63,'rgba(6,10,17,1)');forged.addColorStop(1,'rgba(0,2,6,1)');context.fillStyle=forged;context.fillRect(31,31,66,66);
      const facetShade=context.createLinearGradient(39,35,89,91);facetShade.addColorStop(0,'rgba(232,248,247,.065)');facetShade.addColorStop(.34,'rgba(0,0,0,0)');facetShade.addColorStop(.63,'rgba(0,0,0,.28)');facetShade.addColorStop(1,'rgba(0,0,0,.68)');context.fillStyle=facetShade;context.fillRect(31,31,66,66);
      context.globalCompositeOperation='screen';const ion=context.createLinearGradient(43,88,87,42);ion.addColorStop(.36,'rgba(0,0,0,0)');ion.addColorStop(.47,rgba(color,.055));ion.addColorStop(.495,'rgba(149,235,236,.19)');ion.addColorStop(.515,'rgba(193,126,176,.09)');ion.addColorStop(.57,'rgba(0,0,0,0)');context.fillStyle=ion;context.fillRect(31,31,66,66);context.globalCompositeOperation='source-over';
      context.beginPath();context.moveTo(39,79);context.lineTo(61,43);context.lineTo(88,49);context.closePath();context.fillStyle='rgba(202,238,239,.035)';context.fill();context.beginPath();context.moveTo(39,79);context.lineTo(66,87);context.lineTo(88,49);context.closePath();context.fillStyle=rgba(color,.045);context.fill();
      let grainSeed=stableHash(key);for(let grainIndex=0;grainIndex<18;grainIndex+=1){grainSeed=Math.imul(grainSeed^grainIndex,1664525)+1013904223>>>0;const gy=40+(grainSeed%4800)/100,gx=40+((grainSeed>>>11)%4800)/100;context.fillStyle=grainIndex%6===0?'rgba(218,242,241,.09)':rgba(color,.04);context.fillRect(gx,gy,grainIndex%5===0?1.1:.5,.4);}context.restore();
      traceMaterialShape(context,shape,cx,cy,radius+.45);context.strokeStyle=rgba(color,selected?.82:.48);context.lineWidth=1.05;context.stroke();traceMaterialShape(context,shape,cx,cy,radius-1.35);context.strokeStyle='rgba(220,241,242,.22)';context.lineWidth=.62;context.stroke();traceMaterialShape(context,shape,cx,cy,radius-3.1);context.strokeStyle='rgba(0,0,0,.72)';context.lineWidth=.82;context.stroke();
      context.save();traceMaterialShape(context,shape,cx,cy,radius+.55);context.clip();context.beginPath();context.moveTo(39,48);context.lineTo(58,35);context.lineTo(78,39);context.strokeStyle='rgba(223,248,247,.48)';context.lineWidth=.92;context.stroke();context.beginPath();context.moveTo(85,72);context.lineTo(77,88);context.strokeStyle=rgba(color,.38);context.lineWidth=.64;context.stroke();context.restore();
      if(accent){traceMaterialShape(context,shape,cx,cy,radius+2.5);context.strokeStyle=accent;context.globalAlpha=.52;context.lineWidth=.72;context.stroke();context.globalAlpha=1;}if(selected){traceMaterialShape(context,shape,cx,cy,radius+3.2);context.strokeStyle='rgba(186,241,241,.72)';context.lineWidth=.7;context.stroke();context.fillStyle='rgba(225,253,251,.86)';for(const point of [[42,45],[86,83]])context.fillRect(point[0],point[1],1.4,1.4);}
    ...
    function highlightedEdgeScreenWidth(zoom,selected){if(zoom>=80)return selected ? .55 : .12;if(zoom>=45)return selected ? .62 : .14;if(zoom>=18)return selected ? .72 : .17;return selected ? .9 : .24;}
    function highlightedEdgeOpacity(zoom,selected){if(zoom>=80)return selected ? .68 : .11;if(zoom>=45)return selected ? .7 : .13;if(zoom>=18)return selected ? .72 : .16;return selected ? .74 : .2;}
    ...
    async function runRuntimeProbe(){const pause=milliseconds=>new Promise(resolve=>window.setTimeout(resolve,milliseconds)),endpointDistance=edge=>{const source=state.cy.$id(edge.source),target=state.cy.$id(edge.target);if(!source.length||!target.length)return 0;const a=source.position(),b=target.position();return Math.hypot(a.x-b.x,a.y-b.y);};await pause(450);drawMaterialLayer();await pause(80);const selectedEdge=allEdges.find(edge=>edge.category==='semantic-relation')||allEdges[0];if(!selectedEdge)throw new Error('runtime probe requires at least one relation');const overview={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeEndpointDistance:endpointDistance(selectedEdge)};state.selected={type:'edge',id:selectedEdge.id};renderSelection();highlight();zoomGraphTo(100);await pause(420);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();drawMaterialLayer();await pause(80);const maximum={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeId:selectedEdge.id,selectedEdgeEndpointDistance:endpointDistance(selectedEdge),selectedEndpointMaterialPixels:materialNodePixelEvidence(selectedEdge.source),selectionText:document.getElementById('selectionInspector').innerText};maximum.endpointGeometryScale=overview.selectedEdgeEndpointDistance>0?maximum.selectedEdgeEndpointDistance/overview.selectedEdgeEndpointDistance:0;const checks=[
      ...
      {id:'astral-forged-glass-material-active',passed:maximum.state.materialProfile==='cached-astral-forged-glass-v3',actual:maximum.state.materialProfile,expected:'cached-astral-forged-glass-v3'},
      ...
      {id:'material-viewport-candidates-bounded',passed:maximum.state.materialCandidateCount>0&&maximum.state.materialCandidateCount<allNodes.length,actual:maximum.state.materialCandidateCount,expected:'1..'+(allNodes.length-1)},
      ...
      {id:'renderer-zoom-100',passed:Math.abs(maximum.state.rendererZoom-100)<.001,actual:maximum.state.rendererZoom,expected:100},
      ...
      {id:'selected-edge-screen-width',passed:Math.abs(maximum.state.selectedEdgeRenderedWidth-.55)<.002,actual:maximum.state.selectedEdgeRenderedWidth,expected:.55},
      {id:'selected-edge-opacity',passed:Math.abs(maximum.state.selectedEdgeRenderedOpacity-.68)<.002,actual:maximum.state.selectedEdgeRenderedOpacity,expected:.68},
