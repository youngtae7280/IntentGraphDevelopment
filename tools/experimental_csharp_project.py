...
def build_projection(project_workspace: Path) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    projection = {
        ...
        "uiContract": {
            ...
            "maximumZoom": 256,
            "rendererMaximumZoom": 256,
            "actualCameraMaximumZoom": True,
            "virtualPrecisionZoom": False,
            "effectiveGeometryMaximumZoom": 256,
            "virtualGeometryScaleAtMaximumZoom": 1.0,
            "selectedEdgeRenderedWidthPixelsAtMaximumZoom": 0.18,
            ...
            "stellarVitreousNodeMaterial": True,
...
<body>
  <div class="app">
    ...
    <div class="workspace">
      ...
      <main class="canvas">
        <div class="canvasbar"><div><h1 id="graphTitle">Full project graph</h1><p id="graphSummary"></p></div><div class="tools"><button class="icon" id="zoomOut" title="Zoom out">-</button><output id="zoomReadout" class="zoom-readout" title="Current graph zoom; maximum 256x">x1.0</output><button class="icon" id="zoomIn" title="Zoom in">+</button><button class="icon" id="zoom100" title="Jump to 100x around the current selection">100x</button><button class="icon" id="zoomMax" title="Jump to maximum 256x around the current selection">256x</button><button class="icon" id="fitGraph" title="Fit graph">Fit</button></div></div>
  ...
  <script>
    ...
    const precisionZoomPivot=18,rendererMaximumZoom=256,logicalMaximumZoom=256,materialSpriteCacheLimit=96,materialSpatialCellSize=120;
    ...
    function materialSprite(node,selected,accent){const color=nodeColor(node),shape=materialShape(node),key=['stellar-vitreous-v5',color,shape,selected?'selected':'normal',accent||''].join('|');if(state.materialSpriteCache.has(key))return state.materialSpriteCache.get(key);const canvas=document.createElement('canvas');canvas.width=canvas.height=192;const context=canvas.getContext('2d'),cx=96,cy=96,radius=44;
      // Cached stellar-vitreous optics. Source-space strokes stay intentionally broad so
      // prismatic detail survives the normal 18-32 screen-pixel material footprint.
      const halo=context.createRadialGradient(cx-3,cy-5,radius*.68,cx,cy,78);halo.addColorStop(0,rgba(color,selected?.20:.075));halo.addColorStop(.42,rgba(color,selected?.075:.022));halo.addColorStop(1,rgba(color,0));context.fillStyle=halo;context.fillRect(0,0,192,192);
      context.save();context.shadowColor=selected?rgba(color,.34):'rgba(0,1,6,.98)';context.shadowBlur=selected?22:15;context.shadowOffsetY=4;traceMaterialShape(context,shape,cx,cy,radius+3);context.fillStyle='rgba(0,2,8,.99)';context.fill();context.restore();
      context.save();traceMaterialShape(context,shape,cx,cy,radius);context.clip();
      const core=context.createRadialGradient(77,68,2,102,103,64);core.addColorStop(0,'rgba(66,82,96,.98)');core.addColorStop(.17,'rgba(20,31,44,.99)');core.addColorStop(.52,'rgba(5,10,19,1)');core.addColorStop(.82,'rgba(1,4,11,1)');core.addColorStop(1,'rgba(0,1,6,1)');context.fillStyle=core;context.fillRect(46,46,100,100);
      const nebula=context.createLinearGradient(52,139,144,49);nebula.addColorStop(0,'rgba(147,56,132,.06)');nebula.addColorStop(.34,'rgba(0,0,0,0)');nebula.addColorStop(.455,rgba(color,.16));nebula.addColorStop(.49,'rgba(178,242,246,.34)');nebula.addColorStop(.525,'rgba(105,205,225,.16)');nebula.addColorStop(.565,'rgba(202,83,169,.13)');nebula.addColorStop(.66,'rgba(0,0,0,0)');nebula.addColorStop(1,'rgba(0,0,0,.72)');context.fillStyle=nebula;context.fillRect(44,44,104,104);
      const vignette=context.createRadialGradient(cx,cy,14,cx,cy,52);vignette.addColorStop(0,'rgba(0,0,0,0)');vignette.addColorStop(.68,'rgba(0,0,0,.08)');vignette.addColorStop(1,'rgba(0,0,0,.64)');context.fillStyle=vignette;context.fillRect(43,43,106,106);
      context.globalCompositeOperation='screen';context.lineCap='round';context.beginPath();context.moveTo(59,116);context.bezierCurveTo(73,87,86,63,119,59);context.strokeStyle=rgba(color,.25);context.lineWidth=3.6;context.stroke();context.beginPath();context.moveTo(64,124);context.bezierCurveTo(90,137,119,126,137,91);context.strokeStyle='rgba(113,221,232,.16)';context.lineWidth=2.4;context.stroke();context.beginPath();context.moveTo(65,76);context.lineTo(87,57);context.lineTo(119,61);context.strokeStyle='rgba(231,253,252,.42)';context.lineWidth=4.2;context.stroke();context.beginPath();context.moveTo(126,101);context.lineTo(118,135);context.strokeStyle='rgba(201,82,169,.28)';context.lineWidth=3;context.stroke();
      let grainSeed=stableHash(key);for(let grainIndex=0;grainIndex<16;grainIndex+=1){grainSeed=Math.imul(grainSeed^grainIndex,1664525)+1013904223>>>0;const gy=61+(grainSeed%7000)/100,gx=61+((grainSeed>>>11)%7000)/100,bright=grainIndex%5===0,size=bright?3.2:1.7;context.fillStyle=bright?'rgba(224,250,250,.62)':rgba(color,.22);context.fillRect(gx-size/2,gy-size/2,size,size);if(bright){context.fillStyle='rgba(225,250,251,.25)';context.fillRect(gx-4.2,gy-.65,8.4,1.3);context.fillRect(gx-.65,gy-4.2,1.3,8.4);}}
      context.globalCompositeOperation='source-over';context.restore();
      traceMaterialShape(context,shape,cx,cy,radius+1.9);context.strokeStyle='rgba(0,1,7,.99)';context.lineWidth=7.2;context.stroke();traceMaterialShape(context,shape,cx,cy,radius+.75);context.strokeStyle=rgba(color,selected?.96:.72);context.lineWidth=4.1;context.stroke();traceMaterialShape(context,shape,cx,cy,radius-.95);context.strokeStyle='rgba(185,235,241,.28)';context.lineWidth=2.2;context.stroke();
      context.save();context.globalCompositeOperation='screen';const rim=context.createLinearGradient(55,55,137,137);rim.addColorStop(0,'rgba(211,252,252,.88)');rim.addColorStop(.27,rgba(color,.52));rim.addColorStop(.58,'rgba(83,198,220,.30)');rim.addColorStop(.82,'rgba(205,72,164,.52)');rim.addColorStop(1,'rgba(32,8,35,.18)');traceMaterialShape(context,shape,cx,cy,radius+.75);context.strokeStyle=rim;context.lineWidth=3.6;context.stroke();traceMaterialShape(context,shape,cx,cy,radius-3.1);context.strokeStyle=rgba(color,.35);context.lineWidth=2;context.stroke();context.restore();
      if(accent){context.save();context.globalCompositeOperation='screen';traceMaterialShape(context,shape,cx,cy,radius+3);context.strokeStyle=accent;context.globalAlpha=.62;context.lineWidth=3;context.stroke();context.restore();}if(selected){context.save();context.globalCompositeOperation='screen';traceMaterialShape(context,shape,cx,cy,radius+3.2);context.strokeStyle='rgba(163,239,246,.68)';context.lineWidth=2.2;context.stroke();context.fillStyle='rgba(234,255,254,.96)';for(const point of [[65,69],[130,124]]){context.fillRect(point[0]-1.6,point[1]-1.6,3.2,3.2);}context.restore();}
      if(state.materialSpriteCache.size>=materialSpriteCacheLimit){const oldest=state.materialSpriteCache.keys().next().value;if(oldest!==undefined)state.materialSpriteCache.delete(oldest);}state.materialSpriteCache.set(key,canvas);return canvas;}
    ...
    function drawMaterialLayer(){
      ...
      candidates.forEach(node=>{
        ...
        const selectedEndpoint=Boolean(selectedRelation&&(selectedRelation.source===node.id||selectedRelation.target===node.id)),selected=state.selected?.type==='node'&&state.selected.id===node.id||selectedEndpoint;
        ...
        const modelX=anchor.x+(base.x-anchor.x)*factor,modelY=anchor.y+(base.y-anchor.y)*factor,x=modelX*zoom+pan.x,y=modelY*zoom+pan.y,size=materialNodeSize(node),precisionMaterial=state.viewportScale?.startsWith('precision-'),outer=size*(precisionMaterial?(selected?1.16:1.12):(selected?1.36:1.28));
    ...
    function highlightedEdgeScreenWidth(zoom,selected){if(zoom>=160)return selected ? .18 : .045;if(zoom>=100)return selected ? .22 : .055;if(zoom>=80)return selected ? .26 : .065;if(zoom>=45)return selected ? .32 : .08;if(zoom>=18)return selected ? .42 : .11;return selected ? .70 : .20;}
    function highlightedEdgeOpacity(zoom,selected){if(zoom>=160)return selected ? .34 : .055;if(zoom>=100)return selected ? .38 : .065;if(zoom>=80)return selected ? .42 : .075;if(zoom>=45)return selected ? .48 : .09;if(zoom>=18)return selected ? .56 : .12;return selected ? .68 : .17;}
    ...
    function materialNodePixelEvidence(nodeId){const layer=state.materialLayer,item=state.cy?.$id(nodeId);if(!layer||!item?.length)return {sampleCount:0,opaqueSampleCount:0,chromaticSampleCount:0,uniqueColorBucketCount:0,luminanceRange:0};try{const ratio=layer.width/Math.max(1,state.cy.width()),position=item.renderedPosition(),radius=Math.round(28*ratio),centerX=Math.round(position.x*ratio),centerY=Math.round(position.y*ratio),left=Math.max(0,centerX-radius),top=Math.max(0,centerY-radius),right=Math.min(layer.width,centerX+radius),bottom=Math.min(layer.height,centerY+radius),width=Math.max(0,right-left),height=Math.max(0,bottom-top);if(!width||!height)return {sampleCount:0,opaqueSampleCount:0,chromaticSampleCount:0,uniqueColorBucketCount:0,luminanceRange:0};const pixels=state.materialContext.getImageData(left,top,width,height).data,buckets=new Set();let opaqueSampleCount=0,chromaticSampleCount=0,minLuminance=255,maxLuminance=0;for(let offset=0;offset<pixels.length;offset+=4){const r=pixels[offset],g=pixels[offset+1],b=pixels[offset+2],a=pixels[offset+3];if(a<=12)continue;opaqueSampleCount+=1;if(Math.max(r,g,b)-Math.min(r,g,b)>10)chromaticSampleCount+=1;const luminance=Math.round(.2126*r+.7152*g+.0722*b);minLuminance=Math.min(minLuminance,luminance);maxLuminance=Math.max(maxLuminance,luminance);buckets.add((r>>4)+','+(g>>4)+','+(b>>4)+','+(a>>5));}return {sampleCount:width*height,opaqueSampleCount,chromaticSampleCount,uniqueColorBucketCount:buckets.size,luminanceRange:opaqueSampleCount?maxLuminance-minLuminance:0};}catch(error){runtimeProbeErrors.push('material node sample: '+String(error.message||error));return {sampleCount:0,opaqueSampleCount:0,chromaticSampleCount:0,uniqueColorBucketCount:0,luminanceRange:0};}}
    ...
    async function runRuntimeProbe(){const pause=milliseconds=>new Promise(resolve=>window.setTimeout(resolve,milliseconds)),frames=()=>new Promise(resolve=>window.requestAnimationFrame(()=>window.requestAnimationFrame(resolve))),endpointDistance=edge=>{const source=state.cy.$id(edge.source),target=state.cy.$id(edge.target);if(!source.length||!target.length)return 0;const a=source.position(),b=target.position();return Math.hypot(a.x-b.x,a.y-b.y);},nearestCenterDistance=()=>{const center={x:state.cy.width()/2,y:state.cy.height()/2};let distance=Infinity;state.cy.nodes(':visible').forEach(item=>{const point=item.renderedPosition();distance=Math.min(distance,Math.hypot(point.x-center.x,point.y-center.y));});return distance;};await pause(450);drawMaterialLayer();await pause(80);const selectedEdge=allEdges.find(edge=>edge.category==='semantic-relation')||allEdges[0];if(!selectedEdge)throw new Error('runtime probe requires at least one relation');const overview={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeEndpointDistance:endpointDistance(selectedEdge)};document.getElementById('zoomMax').click();await frames();await pause(80);const unselectedMaximumAnchorDistance=nearestCenterDistance();document.getElementById('fitGraph').click();await frames();await pause(50);const hiddenAnchorNode=state.cy.nodes(':visible')[0],hiddenAnchorDisplay=hiddenAnchorNode?.style('display');if(hiddenAnchorNode?.length){state.selected={type:'node',id:hiddenAnchorNode.id()};hiddenAnchorNode.style('display','none');}document.getElementById('zoomMax').click();await frames();await pause(80);const hiddenSelectionMaximumAnchorDistance=nearestCenterDistance();if(hiddenAnchorNode?.length)hiddenAnchorNode.style('display',hiddenAnchorDisplay||'element');state.selected=null;document.getElementById('fitGraph').click();await frames();await pause(80);state.selected={type:'edge',id:selectedEdge.id};renderSelection();highlight();const hundredStarted=performance.now();document.getElementById('zoom100').click();await frames();drawMaterialLayer();const hundredInteractionMilliseconds=performance.now()-hundredStarted;await pause(90);updateZoomReadout();const hundred=runtimeProbeState(),maximumStarted=performance.now();document.getElementById('zoomMax').click();await frames();drawMaterialLayer();const maximumInteractionMilliseconds=performance.now()-maximumStarted;await pause(110);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();drawMaterialLayer();await pause(60);const panBefore={...state.cy.pan()},panStarted=performance.now();state.cy.pan({x:panBefore.x+18,y:panBefore.y-12});await frames();drawMaterialLayer();const panInteractionMilliseconds=performance.now()-panStarted,panAfter=state.cy.pan(),panDelta={x:panAfter.x-panBefore.x,y:panAfter.y-panBefore.y};state.cy.pan(panBefore);await frames();document.getElementById('zoomOut').click();await frames();await pause(70);updateZoomReadout();const afterZoomOut=runtimeProbeState();document.getElementById('zoomMax').click();await frames();await pause(100);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();drawMaterialLayer();await pause(60);const maximum={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeId:selectedEdge.id,selectedEdgeEndpointDistance:endpointDistance(selectedEdge),selectedEndpointMaterialPixels:materialNodePixelEvidence(selectedEdge.source),selectionText:document.getElementById('selectionInspector').innerText,navigation:{hundred,afterZoomOut,panDelta,unselectedMaximumAnchorDistance,hiddenSelectionMaximumAnchorDistance,hundredInteractionMilliseconds,maximumInteractionMilliseconds,panInteractionMilliseconds}};maximum.endpointGeometryScale=overview.selectedEdgeEndpointDistance>0?maximum.selectedEdgeEndpointDistance/overview.selectedEdgeEndpointDistance:0;const checks=[
      ...
      {id:'stellar-vitreous-material-active',passed:maximum.state.materialProfile==='cached-stellar-vitreous-v5',actual:maximum.state.materialProfile,expected:'cached-stellar-vitreous-v5'},
      ...
      {id:'selected-endpoint-material-detailed',passed:maximum.selectedEndpointMaterialPixels.opaqueSampleCount>=120&&maximum.selectedEndpointMaterialPixels.chromaticSampleCount>=12&&maximum.selectedEndpointMaterialPixels.uniqueColorBucketCount>=8&&maximum.selectedEndpointMaterialPixels.luminanceRange>=35,actual:maximum.selectedEndpointMaterialPixels,expected:'opaque>=120, chromatic>=12, color buckets>=8, luminance range>=35'},
      ...
      {id:'zoom-maximum-control',passed:Math.abs(maximum.state.logicalZoom-256)<.001&&Math.abs(maximum.state.rendererZoom-256)<.001,actual:maximum.state,expected:'logical and renderer zoom 256'},
      ...
      {id:'zoom-out-control',passed:maximum.navigation.afterZoomOut.logicalZoom<256&&maximum.navigation.afterZoomOut.logicalZoom>100,actual:maximum.navigation.afterZoomOut.logicalZoom,expected:'between 100 and 256'},
      ...
      {id:'logical-zoom-256',passed:Math.abs(maximum.state.logicalZoom-256)<.001,actual:maximum.state.logicalZoom,expected:256},
      {id:'renderer-zoom-256',passed:Math.abs(maximum.state.rendererZoom-256)<.001,actual:maximum.state.rendererZoom,expected:256},
      {id:'effective-geometry-zoom-256',passed:Math.abs(maximum.state.effectiveGeometryZoom-256)<.001,actual:maximum.state.effectiveGeometryZoom,expected:256},
      ...
      {id:'selected-edge-screen-width',passed:Math.abs(maximum.state.selectedEdgeRenderedWidth-.18)<.002,actual:maximum.state.selectedEdgeRenderedWidth,expected:.18},
...
def validate_output(output: Path, project_workspace: Path, expected_projection: dict[str, Any], project_before: list[dict[str, str]]) -> dict[str, Any]:
    ...
    for marker in ["id=\"projectGraph\"", "id=\"modeBadge\"", "id=\"workList\"", "id=\"previousWork\"", "id=\"nextWork\"", "id=\"workPosition\"", "id=\"stageList\"", "id=\"previousStage\"", "id=\"nextStage\"", "id=\"stagePosition\"", "id=\"selectionInspector\"", "id=\"changePanel\"", "id=\"deltaList\"", "id=\"evidencePanel\"", "id=\"authorityPanel\"", "id=\"zoomIn\"", "id=\"zoomOut\"", "id=\"zoom100\"", "id=\"zoomMax\"", "id=\"zoomReadout\"", "maxZoom:rendererMaximumZoom", "logicalZoomFromActual", "applyVirtualGeometry", "effectiveGeometryZoom", "selectedEdgeRenderedOpacity", "culledOrdinaryCode", "node-material-layer", "materialSprite", "materialCandidates", "materialNodePixelEvidence", "cached-stellar-vitreous-v5", "zoomAnchorModelPosition", "intentGraphRuntimeProbe", "intentgraph-runtime-probe-report", "graphPixelEvidence", "deepZoomFloors", "precision-", "spectralObsidianOpticalMaterial", "edge:selected", "assets/cytoscape.min.js"]:
