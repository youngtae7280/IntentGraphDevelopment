...
def build_projection(project_workspace: Path) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    projection = {
        ...
        "uiContract": {
            ...
            "selectedEdgeRenderedWidthPixelsAtMaximumZoom": 0.24,
...
<body>
  ...
  <script>
    ...
    const spectralObsidianOpticalMaterial={'background-fill':'solid','background-color':'#070a11','background-blacken':0,'background-opacity':.98,'border-color':'data(color)','border-opacity':.9,'outline-color':'data(color)','outline-opacity':.24,'outline-style':'solid','padding':0,'overlay-opacity':0,'overlay-padding':0};
    function graphStyle(){return [
      {selector:'node',style:{...spectralObsidianOpticalMaterial,'shape':'ellipse','label':'data(label)','color':'#edf7fb','font-family':'Inter,Segoe UI,Arial,sans-serif','font-weight':600,'font-size':9.5,'text-wrap':'ellipsis','text-max-width':126,'text-valign':'bottom','text-margin-y':5,'width':15,'height':15,'border-width':.52,'underlay-color':'data(color)','underlay-opacity':.021,'underlay-padding':1.45,'underlay-shape':'ellipse','text-outline-width':1.35,'text-outline-color':'#030710','text-outline-opacity':.96}},
      {selector:'node[category = "code"]',style:{'label':'','background-fill':'solid','background-color':'data(color)','background-opacity':.86,'background-blacken':.24,'width':2.2,'height':2.2,'opacity':1,'border-width':0,'border-opacity':0,'outline-width':0,'outline-opacity':0,'underlay-opacity':0,'underlay-padding':0}},
      {selector:'node[category = "code"][kind = "file"]',style:{...spectralObsidianOpticalMaterial,'shape':'round-rectangle','width':6.5,'height':6.5,'background-opacity':.98,'border-width':.42,'border-color':'#d8fbff','border-opacity':.68,'underlay-color':'data(color)','underlay-opacity':.026,'underlay-padding':1.05}},
      ...
      {selector:'node[category = "code-capsule"]',style:{...spectralObsidianOpticalMaterial,'background-opacity':.82,'shape':'ellipse','border-width':.46,'border-opacity':.72,'underlay-color':'data(color)','underlay-opacity':.018,'underlay-padding':1.15,'font-size':9,'font-weight':600,'text-max-width':135,'width':19,'height':19,'color':'#dcebf3'}},
    ...
    function highlightedEdgeScreenWidth(zoom,selected){if(zoom>=60)return selected ? .24 : .14;if(zoom>=18)return selected ? .3 : .18;return selected ? .4 : .24;}
    ...
    function updateViewportScale() {
      ...
      const precisionProfile={scale:deepFloor?14/(23*actualZoom):1,node:25,code:23,file:26,namespace:24,type:25,capsule:31,label:13,onDemandLabel:12,labelWidth:162,border:.52,codeBorder:.5,glow:1.45,edge:.88,codeEdge:.88,semanticEdge:1.28};
      ...
      if(precision)dynamicStyles.push({selector:'node',style:{'shape':'ellipse','padding':0,'underlay-opacity':0,'overlay-opacity':0,'overlay-padding':0,'background-fill':'solid','background-color':'#070a11','background-blacken':0,'background-opacity':.98}},{selector:'node[category = "code"]',style:{'shape':'ellipse','background-color':'data(color)','background-blacken':.5,'background-opacity':.94,'border-color':'data(color)','border-opacity':.88,'outline-width':0,'outline-opacity':0}},{selector:'node[category = "code"][kind = "file"]',style:{'outline-width':size(profile.border*.42),'outline-opacity':.18}});
    ...
    function zoomGraphTo(level){const target=Math.max(state.cy.minZoom(),Math.min(logicalMaximumZoom,level)),actualTarget=actualZoomFromLogical(target);if(target>=18)primePrecisionZoom(target);state.cy.zoom({level:actualTarget,renderedPosition:zoomAnchorPosition()});}
    function highlight(){
      if(!state.cy)return;
