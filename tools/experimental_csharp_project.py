...
def build_projection(project_workspace: Path) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    projection = {
        ...
        "uiContract": {
            ...
            "selectedEdgeRenderedWidthPixelsAtMaximumZoom": 0.08,
            "selectedEdgeRenderedOpacityAtMaximumZoom": 0.12,
...
<body>
  ...
  <script>
    ...
    function materialSprite(node,nodeSelected,accent,edgeEndpoint=false){const color=nodeColor(node),shape=materialShape(node),emphasis=nodeSelected?'selected':edgeEndpoint?'edge-endpoint':'normal',key=['nebula-black-metal-v7',color,shape,emphasis,accent||''].join('|');if(state.materialSpriteCache.has(key))return state.materialSpriteCache.get(key);const canvas=document.createElement('canvas');canvas.width=canvas.height=224;const context=canvas.getContext('2d'),cx=112,cy=112,radius=49;
      // Cached black-metal optics: asymmetric facet planes, etched contours, and a thin
      // diffraction rim preserve hierarchy without turning every fact into a glossy orb.
      ...
      const alloy=context.createLinearGradient(72,65,153,158);alloy.addColorStop(0,'rgba(52,68,80,.99)');alloy.addColorStop(.18,'rgba(17,27,38,1)');alloy.addColorStop(.47,'rgba(3,8,16,1)');alloy.addColorStop(.72,'rgba(13,17,29,1)');alloy.addColorStop(1,'rgba(0,2,8,1)');context.fillStyle=alloy;context.fillRect(58,58,108,108);
      ...
      facetPlanes.forEach((points,index)=>{context.beginPath();points.forEach((point,pointIndex)=>pointIndex?context.lineTo(point[0],point[1]):context.moveTo(point[0],point[1]));context.closePath();context.fillStyle=index===0?'rgba(196,239,242,.085)':index===1?rgba(color,.16):index===3?'rgba(116,47,105,.10)':'rgba(78,111,130,.055)';context.fill();});
      ...
      context.save();context.globalCompositeOperation='screen';const rim=context.createLinearGradient(70,67,155,157);rim.addColorStop(0,'rgba(223,253,252,.84)');rim.addColorStop(.18,rgba(color,.56));rim.addColorStop(.56,'rgba(72,164,185,.27)');rim.addColorStop(.78,'rgba(187,67,148,.39)');rim.addColorStop(1,'rgba(20,7,28,.08)');traceMaterialShape(context,shape,cx,cy,radius+.35);context.strokeStyle=rim;context.lineWidth=1.9;context.stroke();traceMaterialShape(context,shape,cx,cy,radius-2.2);context.strokeStyle='rgba(182,231,236,.16)';context.lineWidth=.85;context.stroke();context.restore();
    ...
    function drawMaterialLayer(){
      ...
      candidates.forEach(node=>{
        ...
        context.globalAlpha=node.category==='code'&&state.detailLevel==='overview'?.66:1;context.drawImage(materialSprite(node,nodeSelected,accent,selectedEndpoint),x-outer/2,y-outer/2,outer,outer);drawn+=1;
      });
      context.globalAlpha=1;state.materialDrawCount=drawn;state.materialCulledOrdinaryCodeCount=culledOrdinaryCode;layer.dataset.drawCount=String(drawn);layer.dataset.candidateCount=String(state.materialCandidateCount);layer.dataset.offscreenCulled=String(state.materialOffscreenCulledCount);layer.dataset.culledOrdinaryCode=String(culledOrdinaryCode);layer.dataset.spriteCount=String(state.materialSpriteCache.size);layer.dataset.material='cached-nebula-black-metal-v7';
    }
    ...
    function graphStyle(){return [
      ...
      {selector:'edge:selected',style:{'line-color':'#a8d8dc','opacity':.46,'overlay-opacity':0,'underlay-opacity':0,'z-index':99}},
    ...
    function highlightedEdgeScreenWidth(zoom,selected){if(zoom>=384)return selected ? .08 : .028;if(zoom>=256)return selected ? .10 : .032;if(zoom>=160)return selected ? .12 : .036;if(zoom>=100)return selected ? .14 : .042;if(zoom>=80)return selected ? .17 : .048;if(zoom>=45)return selected ? .21 : .060;if(zoom>=18)return selected ? .28 : .078;return selected ? .52 : .15;}
    function highlightedEdgeOpacity(zoom,selected){if(zoom>=384)return selected ? .12 : .032;if(zoom>=256)return selected ? .14 : .036;if(zoom>=160)return selected ? .16 : .040;if(zoom>=100)return selected ? .18 : .046;if(zoom>=80)return selected ? .21 : .052;if(zoom>=45)return selected ? .25 : .064;if(zoom>=18)return selected ? .31 : .080;return selected ? .46 : .12;}
    ...
    async function runRuntimeProbe(){const pause=milliseconds=>new Promise(resolve=>window.setTimeout(resolve,milliseconds)),frames=()=>new Promise(resolve=>window.requestAnimationFrame(()=>window.requestAnimationFrame(resolve))),endpointDistance=edge=>{const source=state.cy.$id(edge.source),target=state.cy.$id(edge.target);if(!source.length||!target.length)return 0;const a=source.position(),b=target.position();return Math.hypot(a.x-b.x,a.y-b.y);},nearestCenterDistance=()=>{const center={x:state.cy.width()/2,y:state.cy.height()/2};let distance=Infinity;state.cy.nodes(':visible').forEach(item=>{const point=item.renderedPosition();distance=Math.min(distance,Math.hypot(point.x-center.x,point.y-center.y));});return distance;};await pause(450);drawMaterialLayer();await pause(80);const selectedEdge=allEdges.find(edge=>edge.category==='semantic-relation')||allEdges[0];if(!selectedEdge)throw new Error('runtime probe requires at least one relation');const overview={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeEndpointDistance:endpointDistance(selectedEdge)};document.getElementById('zoomMax').click();await frames();await pause(80);const unselectedMaximumAnchorDistance=nearestCenterDistance();document.getElementById('fitGraph').click();await frames();await pause(50);const hiddenAnchorNode=state.cy.nodes(':visible')[0],hiddenAnchorDisplay=hiddenAnchorNode?.style('display');if(hiddenAnchorNode?.length){state.selected={type:'node',id:hiddenAnchorNode.id()};hiddenAnchorNode.style('display','none');}document.getElementById('zoomMax').click();await frames();await pause(80);const hiddenSelectionMaximumAnchorDistance=nearestCenterDistance();if(hiddenAnchorNode?.length)hiddenAnchorNode.style('display',hiddenAnchorDisplay||'element');state.selected=null;document.getElementById('fitGraph').click();await frames();await pause(80);state.selected={type:'edge',id:selectedEdge.id};renderSelection();highlight();const hundredStarted=performance.now();document.getElementById('zoom100').click();await frames();drawMaterialLayer();const hundredInteractionMilliseconds=performance.now()-hundredStarted;await pause(90);updateZoomReadout();const hundred=runtimeProbeState(),maximumStarted=performance.now();document.getElementById('zoomMax').click();await frames();drawMaterialLayer();const maximumInteractionMilliseconds=performance.now()-maximumStarted;await pause(110);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();drawMaterialLayer();await pause(60);const panBefore={...state.cy.pan()},panStarted=performance.now();state.cy.pan({x:panBefore.x+18,y:panBefore.y-12});await frames();drawMaterialLayer();const panInteractionMilliseconds=performance.now()-panStarted,panAfter=state.cy.pan(),panDelta={x:panAfter.x-panBefore.x,y:panAfter.y-panBefore.y};state.cy.pan(panBefore);await frames();document.getElementById('zoomOut').click();await frames();await pause(70);updateZoomReadout();const afterZoomOut=runtimeProbeState();document.getElementById('zoomMax').click();await frames();await pause(100);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();drawMaterialLayer();await pause(60);const maximum={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeId:selectedEdge.id,selectedEdgeEndpointDistance:endpointDistance(selectedEdge),selectedEndpointMaterialPixels:materialNodePixelEvidence(selectedEdge.source),selectionText:document.getElementById('selectionInspector').innerText,navigation:{hundred,afterZoomOut,panDelta,unselectedMaximumAnchorDistance,hiddenSelectionMaximumAnchorDistance,hundredInteractionMilliseconds,maximumInteractionMilliseconds,panInteractionMilliseconds}};maximum.endpointGeometryScale=overview.selectedEdgeEndpointDistance>0?maximum.selectedEdgeEndpointDistance/overview.selectedEdgeEndpointDistance:0;const checks=[
      ...
      {id:'nebula-black-metal-material-active',passed:maximum.state.materialProfile==='cached-nebula-black-metal-v7',actual:maximum.state.materialProfile,expected:'cached-nebula-black-metal-v7'},
      ...
      {id:'selected-endpoint-material-compact',passed:maximum.selectedEndpointMaterialPixels.opaqueBoundsWidth>0&&maximum.selectedEndpointMaterialPixels.opaqueBoundsWidth<=22&&maximum.selectedEndpointMaterialPixels.opaqueBoundsHeight>0&&maximum.selectedEndpointMaterialPixels.opaqueBoundsHeight<=22,actual:maximum.selectedEndpointMaterialPixels,expected:'opaque bounds >0 and <=22x22 screen pixels'},
      ...
      {id:'selected-edge-screen-width',passed:Math.abs(maximum.state.selectedEdgeRenderedWidth-.08)<.002,actual:maximum.state.selectedEdgeRenderedWidth,expected:.08},
      {id:'selected-edge-opacity',passed:Math.abs(maximum.state.selectedEdgeRenderedOpacity-.12)<.002,actual:maximum.state.selectedEdgeRenderedOpacity,expected:.12},
...
def validate_output(output: Path, project_workspace: Path, expected_projection: dict[str, Any], project_before: list[dict[str, str]]) -> dict[str, Any]:
    ...
    for marker in ["id=\"projectGraph\"", "id=\"modeBadge\"", "id=\"workList\"", "id=\"previousWork\"", "id=\"nextWork\"", "id=\"workPosition\"", "id=\"stageList\"", "id=\"previousStage\"", "id=\"nextStage\"", "id=\"stagePosition\"", "id=\"selectionInspector\"", "id=\"changePanel\"", "id=\"deltaList\"", "id=\"evidencePanel\"", "id=\"authorityPanel\"", "id=\"zoomIn\"", "id=\"zoomOut\"", "id=\"zoom100\"", "id=\"zoom256\"", "id=\"zoomMax\"", "id=\"zoomReadout\"", "maxZoom:rendererMaximumZoom", "logicalZoomFromActual", "applyVirtualGeometry", "effectiveGeometryZoom", "selectedEdgeRenderedOpacity", "culledOrdinaryCode", "node-material-layer", "materialSprite", "materialCandidates", "materialNodePixelEvidence", "cached-nebula-black-metal-v7", "zoomAnchorModelPosition", "intentGraphRuntimeProbe", "intentgraph-runtime-probe-report", "graphPixelEvidence", "deepZoomFloors", "precision-", "spectralObsidianOpticalMaterial", "edge:selected", "assets/cytoscape.min.js"]:
