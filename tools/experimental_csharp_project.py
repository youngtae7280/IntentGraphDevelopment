...
def build_projection(project_workspace: Path) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    projection = {
        ...
        "uiContract": {
            ...
            "maximumZoom": 512,
            "rendererMaximumZoom": 512,
            "actualCameraMaximumZoom": True,
            "virtualPrecisionZoom": False,
            "effectiveGeometryMaximumZoom": 512,
            "virtualGeometryScaleAtMaximumZoom": 1.0,
            "selectedEdgeRenderedWidthPixelsAtMaximumZoom": 2.0,
            "selectedEdgeRenderedOpacityAtMaximumZoom": 0.90,
            "overviewOrdinaryCodeOpacity": 1.0,
            "overviewCodeRelationOpacity": 0.70,
            "selectedEdgeScreenSpaceTaper": False,
            "graphifyCommunityDotRendering": True,
            "graphifyCategoricalPalette": True,
            "degreeWeightedNodeSizing": True,
            "hubOnlyDefaultLabels": True,
            "confidenceStyledRelations": True,
            "singleRendererCoordinateSpace": True,
            "continuousEdgesDuringViewportInteraction": True,
            "spectralObsidianNodeMaterial": False,
            "iridescentVoidNodeMaterial": False,
            "spectralTitaniumNodeMaterial": False,
            "astralForgedGlassNodeMaterial": False,
            "celestialCeramicNodeMaterial": False,
            "stellarVitreousNodeMaterial": False,
            "nebulaBlackMetalNodeMaterial": False,
            "luminousAlloyHierarchyContrast": False,
            "browserRuntimeProbe": True,
            "headlessBrowserRegression": True,
            "canvasPixelEvidence": True,
            "rendererSafeOpticalMaterial": False,
            "cachedCanvasNodeMaterial": False,
            "boundedMaterialSpriteCache": False,
            "viewportLocalMaterialRendering": False,
            "viewportSpatialMaterialIndex": False,
            "farZoomMaterialCulling": False,
            "selectedMaterialPixelEvidence": False,
...
<body>
  <div class="app">
    ...
    <div class="workspace">
      ...
      <main class="canvas">
        <div class="canvasbar"><div><h1 id="graphTitle">Full project graph</h1><p id="graphSummary"></p></div><div class="tools"><button class="icon" id="zoomOut" title="Zoom out">-</button><output id="zoomReadout" class="zoom-readout" title="Current graph zoom; maximum 512x">x1.0</output><button class="icon" id="zoomIn" title="Zoom in">+</button><button class="icon" id="zoom100" title="Jump to 100x around the current selection">100x</button><button class="icon" id="zoom256" title="Jump to 256x around the current selection">256x</button><button class="icon" id="zoomMax" title="Jump to maximum 512x around the current selection">512x</button><button class="icon" id="fitGraph" title="Fit graph">Fit</button></div></div>
  ...
  <script>
    ...
    const nodeDegrees = new Map(allNodes.map(node=>[node.id,0]));
    allEdges.forEach(edge=>{nodeDegrees.set(edge.source,(nodeDegrees.get(edge.source)||0)+1);nodeDegrees.set(edge.target,(nodeDegrees.get(edge.target)||0)+1);});
    const maximumNodeDegree=Math.max(1,...nodeDegrees.values()),hubLabelThreshold=Math.max(2,maximumNodeDegree*.15);
    ...
    const colors = { code:'#4E79A7', project:'#76B7B2', 'source-document':'#BAB0AC', goal:'#EDC948', capability:'#59A14F', constraint:'#F28E2B', 'verification-requirement':'#B07AA1', work:'#59A14F', intent:'#FF9DA7', mapping:'#EDC948', proposal:'#E15759', verification:'#B07AA1', evidence:'#76B7B2', 'review-receipt':'#B07AA1', 'verifier-result':'#76B7B2', 'evidence-decision':'#EDC948', authority:'#4E79A7', history:'#9C755F' };
    const communityPalette = ['#4E79A7','#F28E2B','#E15759','#76B7B2','#59A14F','#EDC948','#B07AA1','#FF9DA7','#9C755F','#BAB0AC'];
    ...
    function sourceModule(node) { if(node.category==='code-capsule')return node.label;const parts=String(node.source?.file||'Unmapped').replace(/\\/g,'/').split('/').filter(Boolean);return ['src','tests','test','samples','examples'].includes(String(parts[0]||'').toLowerCase())&&parts[1]?parts[1]:(parts[0]||'Unmapped'); }
    ...
    function materialShape(node){if(['file','source-document','work'].includes(node.kind)||['source-document','work'].includes(node.category))return 'squircle';if(['type','project','code-capsule','proposal'].includes(node.kind)||['project','code-capsule','proposal'].includes(node.category))return 'hex';if(['verification','evidence','verifier-result','evidence-decision','authority','history','verification-requirement'].includes(node.category))return 'diamond';return 'facet';}
    function traceMaterialShape(context,shape,cx,cy,radius){context.beginPath();if(shape==='squircle'){const left=cx-radius,right=cx+radius,top=cy-radius,bottom=cy+radius,corner=radius*.28;context.moveTo(left+corner,top);context.lineTo(right-corner,top);context.quadraticCurveTo(right,top,right,top+corner);context.lineTo(right,bottom-corner);context.quadraticCurveTo(right,bottom,right-corner,bottom);context.lineTo(left+corner,bottom);context.quadraticCurveTo(left,bottom,left,bottom-corner);context.lineTo(left,top+corner);context.quadraticCurveTo(left,top,left+corner,top);}else if(shape==='hex'){for(let index=0;index<6;index+=1){const angle=Math.PI/3*index-Math.PI/2,point={x:cx+Math.cos(angle)*radius,y:cy+Math.sin(angle)*radius};if(index===0)context.moveTo(point.x,point.y);else context.lineTo(point.x,point.y);}}else if(shape==='diamond'){context.moveTo(cx,cy-radius);context.lineTo(cx+radius*.78,cy);context.lineTo(cx,cy+radius);context.lineTo(cx-radius*.78,cy);}else{for(let index=0;index<8;index+=1){const angle=Math.PI/4*index-Math.PI/8,point={x:cx+Math.cos(angle)*radius,y:cy+Math.sin(angle)*radius};if(index===0)context.moveTo(point.x,point.y);else context.lineTo(point.x,point.y);}}context.closePath();}
    function materialSprite(node,nodeSelected,accent,edgeEndpoint=false){const color=nodeColor(node),shape=materialShape(node),isCode=node.category==='code',emphasis=nodeSelected?'selected':edgeEndpoint?'edge-endpoint':'normal',key=['luminous-nebula-alloy-v9',color,shape,emphasis,accent||''].join('|');if(state.materialSpriteCache.has(key))return state.materialSpriteCache.get(key);const canvas=document.createElement('canvas');canvas.width=canvas.height=224;const context=canvas.getContext('2d'),cx=112,cy=112,radius=49;
      // Cached luminous-alloy optics: asymmetric facet planes, etched contours, and a
      // thin diffraction rim preserve hierarchy without turning every fact into a glossy orb.
      const halo=context.createRadialGradient(cx-5,cy-6,radius*.78,cx,cy,82);halo.addColorStop(0,rgba(color,nodeSelected?.12:edgeEndpoint?.045:.025));halo.addColorStop(.52,rgba(color,nodeSelected?.035:.01));halo.addColorStop(1,rgba(color,0));context.fillStyle=halo;context.fillRect(0,0,224,224);
      context.save();context.shadowColor=nodeSelected?rgba(color,.26):'rgba(0,1,7,.94)';context.shadowBlur=nodeSelected?14:7;context.shadowOffsetY=3;traceMaterialShape(context,shape,cx,cy,radius+1.5);context.fillStyle='rgba(0,2,8,.99)';context.fill();context.restore();
      context.save();traceMaterialShape(context,shape,cx,cy,radius);context.clip();
       const alloy=context.createLinearGradient(72,65,153,158);alloy.addColorStop(0,rgba(color,isCode?.68:.74));alloy.addColorStop(.17,'rgba(42,60,76,1)');alloy.addColorStop(.46,'rgba(8,20,34,1)');alloy.addColorStop(.72,rgba(color,isCode?.24:.32));alloy.addColorStop(1,'rgba(2,7,15,1)');context.fillStyle=alloy;context.fillRect(58,58,108,108);
      const facetPlanes=[[[64,77],[103,62],[92,113]],[[103,62],[158,78],[118,112]],[[64,77],[92,113],[71,151]],[[92,113],[118,112],[104,161]],[[118,112],[158,78],[151,145]]];
       facetPlanes.forEach((points,index)=>{context.beginPath();points.forEach((point,pointIndex)=>pointIndex?context.lineTo(point[0],point[1]):context.moveTo(point[0],point[1]));context.closePath();context.fillStyle=index===0?'rgba(225,250,250,.20)':index===1?rgba(color,isCode?.38:.48):index===3?'rgba(176,74,146,.20)':'rgba(111,162,184,.14)';context.fill();});
      context.globalCompositeOperation='screen';context.lineCap='square';
       const signal=context.createLinearGradient(70,139,150,77);signal.addColorStop(0,rgba(color,0));signal.addColorStop(.38,rgba(color,isCode?.32:.42));signal.addColorStop(.52,'rgba(232,255,254,.66)');signal.addColorStop(.66,rgba(color,isCode?.38:.50));signal.addColorStop(1,rgba(color,0));context.beginPath();context.moveTo(71,139);context.lineTo(82,146);context.lineTo(153,83);context.lineTo(145,75);context.closePath();context.fillStyle=signal;context.fill();
      context.beginPath();context.moveTo(68,142);context.lineTo(93,113);context.lineTo(137,75);context.strokeStyle=rgba(color,.20);context.lineWidth=2.1;context.stroke();
      context.beginPath();context.moveTo(79,68);context.lineTo(103,63);context.lineTo(149,79);context.strokeStyle='rgba(219,251,251,.32)';context.lineWidth=1.5;context.stroke();
      context.beginPath();context.moveTo(103,160);context.lineTo(119,113);context.lineTo(153,142);context.strokeStyle='rgba(186,74,157,.13)';context.lineWidth=1.4;context.stroke();
      for(let contour=0;contour<3;contour+=1){traceMaterialShape(context,shape,cx,cy,radius-10-contour*7);context.strokeStyle=contour===0?rgba(color,.085):'rgba(161,210,220,.035)';context.lineWidth=.9;context.stroke();}
      let grainSeed=stableHash(key);for(let grainIndex=0;grainIndex<12;grainIndex+=1){grainSeed=Math.imul(grainSeed^grainIndex,1664525)+1013904223>>>0;const gx=73+((grainSeed>>>9)%7600)/100,gy=73+(grainSeed%7600)/100,size=grainIndex%6===0?1.8:.85;context.fillStyle=grainIndex%6===0?'rgba(224,250,250,.42)':rgba(color,.14);context.fillRect(gx-size/2,gy-size/2,size,size);}
      context.globalCompositeOperation='source-over';context.restore();
      traceMaterialShape(context,shape,cx,cy,radius+1.5);context.strokeStyle='rgba(0,1,7,.98)';context.lineWidth=4.4;context.stroke();
      context.save();context.globalCompositeOperation='screen';const rim=context.createLinearGradient(70,67,155,157);rim.addColorStop(0,'rgba(231,255,254,.92)');rim.addColorStop(.18,rgba(color,.78));rim.addColorStop(.56,'rgba(78,183,205,.40)');rim.addColorStop(.78,'rgba(202,77,159,.48)');rim.addColorStop(1,'rgba(20,7,28,.12)');traceMaterialShape(context,shape,cx,cy,radius+.35);context.strokeStyle=rim;context.lineWidth=2.15;context.stroke();traceMaterialShape(context,shape,cx,cy,radius-2.2);context.strokeStyle=rgba(color,.25);context.lineWidth=.9;context.stroke();context.restore();
      if(accent){context.save();context.globalCompositeOperation='screen';traceMaterialShape(context,shape,cx,cy,radius+2.2);context.strokeStyle=accent;context.globalAlpha=edgeEndpoint?.34:.48;context.lineWidth=edgeEndpoint?1.1:1.55;context.stroke();context.restore();}
      if(nodeSelected){context.save();context.globalCompositeOperation='screen';traceMaterialShape(context,shape,cx,cy,radius+3.1);context.strokeStyle='rgba(174,239,243,.72)';context.lineWidth=1.35;context.stroke();context.fillStyle='rgba(232,255,253,.88)';for(const point of [[77,78],[147,146]]){context.fillRect(point[0]-1.1,point[1]-1.1,2.2,2.2);}context.restore();}
      if(edgeEndpoint){context.save();context.globalCompositeOperation='screen';context.strokeStyle='rgba(153,224,230,.46)';context.lineWidth=1.15;for(const angle of [Math.PI*.25,Math.PI*1.25]){const x1=cx+Math.cos(angle)*(radius+1),y1=cy+Math.sin(angle)*(radius+1),x2=cx+Math.cos(angle)*(radius+7),y2=cy+Math.sin(angle)*(radius+7);context.beginPath();context.moveTo(x1,y1);context.lineTo(x2,y2);context.stroke();}context.restore();}
      const cropSize=edgeEndpoint?168:144,cropOffset=(224-cropSize)/2,sprite=document.createElement('canvas');sprite.width=sprite.height=cropSize;sprite.getContext('2d').drawImage(canvas,cropOffset,cropOffset,cropSize,cropSize,0,0,cropSize,cropSize);
      if(state.materialSpriteCache.size>=materialSpriteCacheLimit){const oldest=state.materialSpriteCache.keys().next().value;if(oldest!==undefined)state.materialSpriteCache.delete(oldest);}state.materialSpriteCache.set(key,sprite);return sprite;}
    ...
    function materialNodeSize(node){const profileState=state.materialProfile;if(!profileState)return node.category==='code'?3.8:16;const {profile,scale,actualZoom}=profileState;let value=profile.node;if(node.category==='code-capsule')value=profile.capsule;else if(node.category==='code'){value=profile.code;if(node.kind==='file')value=profile.file;else if(node.kind==='namespace')value=profile.namespace;else if(node.kind==='type')value=profile.type;}return Math.max(node.category==='code'?3.8:9,Math.min(node.category==='code'?25:34,value*scale*actualZoom));}
    function materialCandidates(positions,width,height,zoom,pan,factor,anchor){
      if(!state.materialSpatialIndex){const buckets=new Map();allNodes.forEach(node=>{const point=positions.get(node.id);if(!point)return;const key=Math.floor(point.x/materialSpatialCellSize)+','+Math.floor(point.y/materialSpatialCellSize);if(!buckets.has(key))buckets.set(key,[]);buckets.get(key).push(node);});state.materialSpatialIndex=buckets;}
      const margin=58,projectedLeft=(-pan.x-margin)/zoom,projectedRight=(width-pan.x+margin)/zoom,projectedTop=(-pan.y-margin)/zoom,projectedBottom=(height-pan.y+margin)/zoom,toBase=(value,origin)=>origin+(value-origin)/Math.max(.0001,factor),left=Math.min(toBase(projectedLeft,anchor.x),toBase(projectedRight,anchor.x)),right=Math.max(toBase(projectedLeft,anchor.x),toBase(projectedRight,anchor.x)),top=Math.min(toBase(projectedTop,anchor.y),toBase(projectedBottom,anchor.y)),bottom=Math.max(toBase(projectedTop,anchor.y),toBase(projectedBottom,anchor.y)),nodes=[];
      for(let cellX=Math.floor(left/materialSpatialCellSize);cellX<=Math.floor(right/materialSpatialCellSize);cellX+=1){for(let cellY=Math.floor(top/materialSpatialCellSize);cellY<=Math.floor(bottom/materialSpatialCellSize);cellY+=1){const bucket=state.materialSpatialIndex.get(cellX+','+cellY);if(bucket)nodes.push(...bucket);}}
      return nodes;
    }
    function drawMaterialLayer(){
      ...
      const zoom=state.cy.zoom(),pan=state.cy.pan(),positions=state.positions||(state.positions=buildPresetPositions()),factor=state.virtualGeometryScale||1,anchor=positions.get(state.virtualGeometryAnchorId)||{x:0,y:0},farMaterial=state.detailLevel==='overview',candidates=materialCandidates(positions,width,height,zoom,pan,factor,anchor);let drawn=0,culledOrdinaryCode=0;
      state.materialCandidateCount=candidates.length;state.materialOffscreenCulledCount=Math.max(0,allNodes.length-candidates.length);
      const selectedRelation=state.selected?.type==='edge'?edgeById.get(state.selected.id):null;
      candidates.forEach(node=>{
        if(state.visibleNodeIds&&!state.visibleNodeIds.has(node.id))return;const base=positions.get(node.id);if(!base)return;
        const selectedEndpoint=Boolean(selectedRelation&&(selectedRelation.source===node.id||selectedRelation.target===node.id)),nodeSelected=state.selected?.type==='node'&&state.selected.id===node.id,selected=nodeSelected||selectedEndpoint;
        const accent=selectedEndpoint?'#8de8ee':state.stageAddedNodeIds.has(node.id)?'#71ead8':state.stageChangedNodeIds.has(node.id)?'#f0a36d':state.searchMatchNodeIds.has(node.id)?'#8de0e8':state.emphasizedNodeIds.has(node.id)?'#d7b56c':'';
        ...
        const modelX=anchor.x+(base.x-anchor.x)*factor,modelY=anchor.y+(base.y-anchor.y)*factor,x=modelX*zoom+pan.x,y=modelY*zoom+pan.y,size=materialNodeSize(node),precisionMaterial=state.viewportScale?.startsWith('precision-'),endpointScale=selectedEndpoint?(precisionMaterial ? .72 : .82):1,outer=size*endpointScale*(precisionMaterial?(nodeSelected?1.12:1.06):(nodeSelected?1.22:1.14));
        if(x+outer<0||x-outer>width||y+outer<0||y-outer>height)return;
        context.globalAlpha=node.category==='code'&&state.detailLevel==='overview'?.96:1;context.drawImage(materialSprite(node,nodeSelected,accent,selectedEndpoint),x-outer/2,y-outer/2,outer,outer);drawn+=1;
      });
      context.globalAlpha=1;state.materialDrawCount=drawn;state.materialCulledOrdinaryCodeCount=culledOrdinaryCode;layer.dataset.drawCount=String(drawn);layer.dataset.candidateCount=String(state.materialCandidateCount);layer.dataset.offscreenCulled=String(state.materialOffscreenCulledCount);layer.dataset.culledOrdinaryCode=String(culledOrdinaryCode);layer.dataset.spriteCount=String(state.materialSpriteCache.size);layer.dataset.material='cached-luminous-nebula-alloy-v9';
    }
    function scheduleMaterialLayer(){return;}
    ...
    function buildPresetPositions() {
      ...
      orderedGroups.forEach(([group,nodes])=>{
        ...
        const fileScale=34+Math.min(34,Math.sqrt(nodes.length)*.9);
        [...files.entries()].sort((left,right)=>left[0].localeCompare(right[0])).forEach(([file,fileNodes],fileIndex)=>{
          ...
          fileNodes.slice().sort((left,right)=>(left.kind+'|'+left.id).localeCompare(right.kind+'|'+right.id)).forEach((node,nodeIndex)=>{
            ...
            const local=spiralPoint(nodeIndex,6.2,stableHash(node.id)%628/100);
    ...
    function normalizedEdgeConfidence(value){const confidence=String(value||'unknown').toLowerCase();return ['extracted','inferred','ambiguous'].includes(confidence)?confidence:'unknown';}
    function graphElements(nodes,edges) {
      const positions=state.positions||(state.positions=buildPresetPositions());
      return [
        ...nodes.map(node=>{const degree=nodeDegrees.get(node.id)||0,labelMode=alwaysLabeledCategories.has(node.category)||degree>=hubLabelThreshold?'always':'on-demand';return {data:{id:node.id,label:node.label,labelMode,degree,category:node.category,kind:node.kind,deltaState:node.deltaState||'unchanged',color:nodeColor(node),community:node.category==='code'||node.category==='code-capsule'?sourceModule(node):node.category},position:positions.get(node.id)||{x:0,y:0}};}),
        ...edges.map(edge=>{const source=nodeById.get(edge.source),confidence=normalizedEdgeConfidence(edge.details?.confidence);return {data:{id:edge.id,source:edge.source,target:edge.target,kind:edge.kind,category:edge.category,confidence,color:edge.category==='code-relation'&&source?nodeColor(source):'#718096'}};})
      ];
    }
    ...
    const spectralObsidianOpticalMaterial={'background-fill':'solid','background-color':'data(color)','background-blacken':0,'background-opacity':1,'border-color':'data(color)','border-opacity':1,'outline-opacity':0,'padding':0,'overlay-opacity':0,'overlay-padding':0};
    function graphStyle(){return [
      {selector:'node',style:{...spectralObsidianOpticalMaterial,'shape':'ellipse','label':'data(label)','color':'#ffffff','font-family':'Inter,Segoe UI,Arial,sans-serif','font-weight':500,'font-size':12,'text-wrap':'ellipsis','text-max-width':150,'text-valign':'bottom','text-margin-y':5,'width':`mapData(degree,0,${maximumNodeDegree},10,40)`,'height':`mapData(degree,0,${maximumNodeDegree},10,40)`,'border-width':1.5,'underlay-opacity':0,'text-outline-width':2,'text-outline-color':'#0f0f1a','text-outline-opacity':.92}},
      {selector:'node[category = "code"]',style:{'label':'','shape':'ellipse','background-color':'data(color)','background-opacity':1,'background-blacken':0,'opacity':1,'border-width':0,'outline-width':0,'underlay-opacity':0}},
      {selector:'node[category = "code"][kind = "file"]',style:{...spectralObsidianOpticalMaterial,'shape':'ellipse','border-width':1}},
      {selector:'node[category = "code"][kind = "namespace"]',style:{...spectralObsidianOpticalMaterial,'shape':'ellipse','border-width':.8}},
      {selector:'node[category = "code"][kind = "type"]',style:{...spectralObsidianOpticalMaterial,'shape':'ellipse','border-width':.9}},
      {selector:'node[category = "code-capsule"]',style:{...spectralObsidianOpticalMaterial,'shape':'ellipse','border-width':1.5,'font-size':12,'font-weight':500,'text-max-width':150,'color':'#ffffff'}},
      ...
      {selector:'node.low-detail',style:{'opacity':.82}},
      {selector:'node[category = "code"].low-detail',style:{'opacity':1}},
      {selector:'node[deltaState = "proposed-change"]',style:{'border-width':2,'border-color':'#e37c74','border-opacity':1}},
      {selector:'edge',style:{'width':1,'line-color':'data(color)','target-arrow-shape':'none','curve-style':'straight','opacity':.35,'line-cap':'round'}},
      {selector:'edge[confidence = "extracted"]',style:{'width':2,'line-style':'solid','opacity':.7}},
      {selector:'edge[confidence = "inferred"]',style:{'width':1,'line-style':'dashed','opacity':.35}},
      {selector:'edge[confidence = "ambiguous"]',style:{'width':1,'line-style':'dashed','opacity':.35}},
      {selector:'edge[confidence = "unknown"]',style:{'width':1,'line-style':'dashed','opacity':.35}},
      {selector:'edge[category = "code-relation"]',style:{'line-color':'data(color)'}},
      {selector:'edge.low-detail',style:{'opacity':.18}},
      {selector:'edge.detail-edge',style:{'width':1.25,'opacity':.52}},
      {selector:'edge[category = "capsule-relation"]',style:{'width':1.4,'opacity':.58}},
      ...
      {selector:'node:selected',style:{'background-color':'#ffffff','background-opacity':1,'border-color':'data(color)','border-width':2,'border-opacity':1,'underlay-opacity':0,'overlay-opacity':0,'z-index':99}},
      {selector:'edge:selected',style:{'line-color':'#ffffff','line-style':'solid','width':2,'opacity':.9,'overlay-opacity':0,'underlay-opacity':0,'z-index':99}},
      {selector:'edge.selection-neighbor',style:{'line-color':'#d9e5f2','line-style':'solid','width':1.25,'opacity':.72,'overlay-opacity':0,'underlay-opacity':0,'z-index':98}}
    ];}
    ...
    function updateSemanticEmphasis() {
      const active=!state.selected&&state.mode==='overview'&&!filters().search&&!filters().category&&!filters().relation;
    ...
    function updateStageHighlight() {
      const stage=!state.selected&&state.stageFocus,added=stage?new Set(stage.addedNodeIds):emptyIds,changed=stage?new Set(stage.changedNodeIds):emptyIds,edges=stage?new Set(stage.edgeIds):emptyIds;
    ...
    function renderGraph(options={}) {
      ...
      if(!state.cy){
        ...
        state.cy=cytoscape({
          ...
          minZoom:.10,maxZoom:rendererMaximumZoom,pixelRatio:1,textureOnViewport:true,motionBlur:false,hideEdgesOnViewport:false,boxSelectionEnabled:false,autoungrabify:true
        });
    ...
    function semanticZoom() {
      ...
      state.viewportStyleZoom=null;updateViewportScale();
      updateOnDemandLabels();
    }
    ...
    function zoomAnchorModelPosition(){
      if(state.selected?.type==='node'){const item=state.cy.$id(state.selected.id);if(item?.length&&item.visible())return item.position();}
      if(state.selected?.type==='edge'){
        const edge=edgeById.get(state.selected.id),edgeItem=state.cy.$id(state.selected.id),source=edge?state.cy.$id(edge.source):null;
        if(edgeItem?.length&&edgeItem.visible()&&source?.length&&source.visible())return source.position();
      }
      const center={x:state.cy.width()/2,y:state.cy.height()/2};let nearest=null,nearestDistance=Infinity;
      state.cy.nodes(':visible').forEach(item=>{const point=item.renderedPosition(),distance=Math.hypot(point.x-center.x,point.y-center.y);if(distance<nearestDistance){nearestDistance=distance;nearest=item;}});
      if(nearest?.length)return nearest.position();
      const project=allNodes.find(node=>node.category==='project'),projectItem=project?state.cy.$id(project.id):null;
      return projectItem?.length?projectItem.position():{x:0,y:0};
    }
    function highlightedEdgeScreenWidth(zoom,selected){return selected?2:1.25;}
    function highlightedEdgeOpacity(zoom,selected){return selected?.9:.72;}
    ...
    function updateViewportScale() {
      if(!state.cy)return;
      const actualZoom=state.precisionActualZoomOverride||state.cy.zoom(),zoom=state.precisionZoomOverride||logicalZoomFromActual(actualZoom),deepZoomFloors=[18,24,32,45,60,80,100,160,256,384,512],deepFloor=zoom>=18?deepZoomFloors.reduce((floor,candidate)=>zoom>=candidate?candidate:floor,18):null,band=zoom<.25?'far':zoom<.52?'overview':zoom<1.05?'structure':zoom<1.8?'detail':zoom<3.5?'close':zoom<7?'deep':zoom<18?'inspect':`precision-${deepFloor}`;
      const styleZoomRatio=state.viewportStyleZoom?actualZoom/state.viewportStyleZoom:Infinity;
      if(state.viewportScale===band&&styleZoomRatio>=.92&&styleZoomRatio<=1.08){if(state.materialProfile)state.materialProfile.actualZoom=actualZoom;refreshHighlightedEdgeScale();scheduleMaterialLayer();return;}
      state.viewportScale=band;state.viewportStyleZoom=actualZoom;document.getElementById('projectGraph').dataset.scaleBand=band;
      ...
      const precisionProfile={scale:deepFloor?18.5/(23*actualZoom):1,node:34,code:24,file:28,namespace:26,type:27,capsule:36,label:13,onDemandLabel:12,labelWidth:162,border:.52,codeBorder:.5,glow:1.45,edge:.88,codeEdge:.88,semanticEdge:1.28};
      const precision=band.startsWith('precision-'),profile=precision?precisionProfile:profiles[band],geometryScale=1,size=value=>Math.max(.001,Math.round(value*profile.scale*1000)/1000),minimumNodePixels=precision?6.6:band==='far'?2.5:band==='overview'?3.5:4.5,maximumNodePixels=precision?22:band==='far'?12:band==='overview'?16:22,nodeSize=`mapData(degree,0,${maximumNodeDegree},${(minimumNodePixels/actualZoom).toFixed(5)},${(maximumNodePixels/actualZoom).toFixed(5)})`,screenSize=value=>Math.max(.001,Math.round(value/actualZoom*10000)/10000),dynamicStyles=[
        {selector:'node',style:{'width':nodeSize,'height':nodeSize,'font-size':screenSize(12),'text-max-width':screenSize(150),'border-width':screenSize(1.5),'outline-width':0,'underlay-padding':0,'text-margin-y':screenSize(5),'text-outline-width':screenSize(2)}},
        {selector:'node.show-on-demand-label',style:{'font-size':screenSize(9),'text-max-width':screenSize(130)}},
        {selector:'node[category = "code"]',style:{'width':nodeSize,'height':nodeSize,'border-width':0,'outline-width':0,'outline-opacity':0,'underlay-padding':0}},
        {selector:'node[category = "code"][kind = "file"]',style:{'width':nodeSize,'height':nodeSize,'border-width':screenSize(1),'outline-width':0,'underlay-padding':0}},
        {selector:'node[category = "code"][kind = "namespace"]',style:{'width':nodeSize,'height':nodeSize}},
        {selector:'node[category = "code"][kind = "type"]',style:{'width':nodeSize,'height':nodeSize}},
        {selector:'node[category = "code-capsule"]',style:{'width':nodeSize,'height':nodeSize,'border-width':screenSize(1.5),'underlay-padding':0}},
        {selector:'edge',style:{'width':screenSize(1)}},
        {selector:'edge[confidence = "extracted"]',style:{'width':screenSize(2)}},
        {selector:'edge[confidence = "inferred"]',style:{'width':screenSize(1)}},
        {selector:'edge[confidence = "ambiguous"]',style:{'width':screenSize(1)}},
        {selector:'edge[confidence = "unknown"]',style:{'width':screenSize(1)}},
        {selector:'edge.detail-edge',style:{'width':screenSize(1.25)}},
        {selector:'edge[category = "capsule-relation"]',style:{'width':screenSize(1.4)}},
        {selector:'edge[category = "mapping-relation"]',style:{'width':screenSize(1.5)}},
        {selector:'edge[category = "delta-relation"]',style:{'width':screenSize(1.5)}},
        {selector:'edge.selection-neighbor',style:{'width':screenSize(1.25)}},
        {selector:'edge.semantic-emphasis',style:{'width':screenSize(1.5)}},
        {selector:'edge.stage-delta',style:{'width':screenSize(1.8)}},
      ...
      if(precision)dynamicStyles.push({selector:'node',style:{'shape':'ellipse','padding':0,'underlay-opacity':0,'overlay-opacity':0,'overlay-padding':0,'background-fill':'solid','background-color':'data(color)','background-blacken':0,'background-opacity':1,'border-color':'data(color)','border-opacity':1,'outline-width':0,'outline-opacity':0}},{selector:'node[category = "code"]',style:{'shape':'ellipse','background-color':'data(color)','background-blacken':0,'background-opacity':1,'border-opacity':0,'outline-width':0,'outline-opacity':0}});
    ...
    function zoomGraphTo(level){const target=Math.max(state.cy.minZoom(),Math.min(logicalMaximumZoom,level)),actualTarget=actualZoomFromLogical(target),anchor=zoomAnchorModelPosition(),center={x:state.cy.width()/2,y:state.cy.height()/2};if(target>=18){state.virtualGeometryAnchorId=precisionAnchorId();primePrecisionZoom(target);}state.desiredLogicalZoom=target;state.desiredActualZoom=actualTarget;state.cy.viewport({zoom:actualTarget,pan:{x:center.x-anchor.x*actualTarget,y:center.y-anchor.y*actualTarget}});applyVirtualGeometry(true);}
    function highlight(){
      if(!state.cy)return;
      ...
      if(!state.selected){updateSemanticEmphasis();updateStageHighlight();updateOnDemandLabels();return;}
      const item=state.cy.$id(state.selected.id);if(!item.length)return;
      item.select();const highlightedEdges=state.selected.type==='edge'?item:item.connectedEdges();highlightedEdges.addClass('selection-neighbor');state.highlightedEdgeIds=new Set(highlightedEdges.map(edge=>edge.id()));updateSemanticEmphasis();updateStageHighlight();refreshHighlightedEdgeScale();state.highlighted=state.selected.id;updateOnDemandLabels();
    }
    ...
    function sampledCanvasPixels(canvas){const width=canvas.width||0,height=canvas.height||0,empty={width,height,sampleCount:0,opaqueSampleCount:0,chromaticSampleCount:0,meanOpaqueLuminance:0};if(!width||!height)return empty;try{const context=canvas.getContext('2d',{willReadFrequently:true}),pixels=context.getImageData(0,0,width,height).data,step=Math.max(1,Math.floor(Math.sqrt(width*height/14000)));let sampleCount=0,opaqueSampleCount=0,chromaticSampleCount=0,opaqueLuminanceTotal=0;for(let y=0;y<height;y+=step){for(let x=0;x<width;x+=step){const offset=(y*width+x)*4,r=pixels[offset],g=pixels[offset+1],b=pixels[offset+2],a=pixels[offset+3];sampleCount+=1;if(a>8){opaqueSampleCount+=1;opaqueLuminanceTotal+=.2126*r+.7152*g+.0722*b;}if(a>8&&Math.max(r,g,b)-Math.min(r,g,b)>12)chromaticSampleCount+=1;}}return {width,height,sampleCount,opaqueSampleCount,chromaticSampleCount,meanOpaqueLuminance:opaqueSampleCount?opaqueLuminanceTotal/opaqueSampleCount:0};}catch(error){runtimeProbeErrors.push('canvas sample: '+String(error.message||error));return empty;}}
    function materialNodePixelEvidence(nodeId){const layer=state.materialLayer,item=state.cy?.$id(nodeId),empty={sampleCount:0,opaqueSampleCount:0,chromaticSampleCount:0,uniqueColorBucketCount:0,luminanceRange:0,opaqueBoundsWidth:0,opaqueBoundsHeight:0};if(!layer||!item?.length)return empty;try{const ratio=layer.width/Math.max(1,state.cy.width()),position=item.renderedPosition(),radius=Math.round(28*ratio),centerX=Math.round(position.x*ratio),centerY=Math.round(position.y*ratio),left=Math.max(0,centerX-radius),top=Math.max(0,centerY-radius),right=Math.min(layer.width,centerX+radius),bottom=Math.min(layer.height,centerY+radius),width=Math.max(0,right-left),height=Math.max(0,bottom-top);if(!width||!height)return empty;const pixels=state.materialContext.getImageData(left,top,width,height).data,buckets=new Set();let opaqueSampleCount=0,chromaticSampleCount=0,minLuminance=255,maxLuminance=0,minX=width,minY=height,maxX=-1,maxY=-1;for(let offset=0;offset<pixels.length;offset+=4){const r=pixels[offset],g=pixels[offset+1],b=pixels[offset+2],a=pixels[offset+3];if(a<=12)continue;const pixelIndex=offset/4,pixelX=pixelIndex%width,pixelY=Math.floor(pixelIndex/width);minX=Math.min(minX,pixelX);minY=Math.min(minY,pixelY);maxX=Math.max(maxX,pixelX);maxY=Math.max(maxY,pixelY);opaqueSampleCount+=1;if(Math.max(r,g,b)-Math.min(r,g,b)>10)chromaticSampleCount+=1;const luminance=Math.round(.2126*r+.7152*g+.0722*b);minLuminance=Math.min(minLuminance,luminance);maxLuminance=Math.max(maxLuminance,luminance);buckets.add((r>>4)+','+(g>>4)+','+(b>>4)+','+(a>>5));}return {sampleCount:width*height,opaqueSampleCount,chromaticSampleCount,uniqueColorBucketCount:buckets.size,luminanceRange:opaqueSampleCount?maxLuminance-minLuminance:0,opaqueBoundsWidth:opaqueSampleCount?(maxX-minX+1)/ratio:0,opaqueBoundsHeight:opaqueSampleCount?(maxY-minY+1)/ratio:0};}catch(error){runtimeProbeErrors.push('material node sample: '+String(error.message||error));return empty;}}
    function selectedEdgeLinePixelEvidence(edge){const empty={sampleCount:0,observedSampleCount:0,longestMissingRun:0,coverage:0,visibleSegmentLength:0};try{const source=state.cy.$id(edge.source),target=state.cy.$id(edge.target);if(!source.length||!target.length)return empty;const start=source.renderedPosition(),end=target.renderedPosition(),length=Math.hypot(end.x-start.x,end.y-start.y);if(!length)return empty;const direction={x:(end.x-start.x)/length,y:(end.y-start.y)/length},sourceRadius=Math.max(parseFloat(source.renderedStyle('width'))||0,parseFloat(source.renderedStyle('height'))||0)/2,targetRadius=Math.max(parseFloat(target.renderedStyle('width'))||0,parseFloat(target.renderedStyle('height'))||0)/2,width=state.cy.width(),height=state.cy.height(),boundaryDistances=[];if(direction.x>0)boundaryDistances.push((width-start.x)/direction.x);else if(direction.x<0)boundaryDistances.push((0-start.x)/direction.x);if(direction.y>0)boundaryDistances.push((height-start.y)/direction.y);else if(direction.y<0)boundaryDistances.push((0-start.y)/direction.y);const startDistance=sourceRadius+3,endDistance=Math.min(length-targetRadius-3,...boundaryDistances.filter(value=>Number.isFinite(value)&&value>0))-3,visibleSegmentLength=Math.max(0,endDistance-startDistance);if(visibleSegmentLength<8)return {...empty,visibleSegmentLength};const canvases=[...document.querySelectorAll('#projectGraph canvas')].filter(canvas=>!String(canvas.className).includes('node-material-layer')),sampleCount=Math.min(2049,Math.max(25,Math.ceil(visibleSegmentLength)+1));let observedSampleCount=0,longestMissingRun=0,missingRun=0;for(let index=0;index<sampleCount;index+=1){const distance=startDistance+visibleSegmentLength*(index/(sampleCount-1)),x=start.x+direction.x*distance,y=start.y+direction.y*distance;const observed=canvases.some(canvas=>{const ratioX=canvas.width/Math.max(1,width),ratioY=canvas.height/Math.max(1,height),cx=Math.round(x*ratioX),cy=Math.round(y*ratioY),radiusX=Math.max(1,Math.round(1.25*ratioX)),radiusY=Math.max(1,Math.round(1.25*ratioY)),left=Math.max(0,cx-radiusX),top=Math.max(0,cy-radiusY),right=Math.min(canvas.width,cx+radiusX+1),bottom=Math.min(canvas.height,cy+radiusY+1);if(right<=left||bottom<=top)return false;const pixels=canvas.getContext('2d',{willReadFrequently:true}).getImageData(left,top,right-left,bottom-top).data;for(let offset=0;offset<pixels.length;offset+=4){const r=pixels[offset],g=pixels[offset+1],b=pixels[offset+2],a=pixels[offset+3],luminance=.2126*r+.7152*g+.0722*b;if(a>8&&luminance>105&&Math.max(r,g,b)-Math.min(r,g,b)<90)return true;}return false;});if(observed){observedSampleCount+=1;missingRun=0;}else{missingRun+=1;longestMissingRun=Math.max(longestMissingRun,missingRun);}}return {sampleCount,observedSampleCount,longestMissingRun,coverage:observedSampleCount/sampleCount,visibleSegmentLength};}catch(error){runtimeProbeErrors.push('selected edge pixel sample: '+String(error.message||error));return empty;}}
    function graphPixelEvidence(){const canvases=[...document.querySelectorAll('#projectGraph canvas')].filter(canvas=>!String(canvas.className).includes('node-material-layer')),layers=canvases.map(canvas=>({className:canvas.className||'',...sampledCanvasPixels(canvas)})),ordinaryCodeNode=allNodes.find(node=>node.category==='code'&&!['file','namespace','type'].includes(node.kind)&&!importantCodeLabelIds.has(node.id)),ordinaryCodeItem=ordinaryCodeNode?state.cy?.$id(ordinaryCodeNode.id):null,ordinaryCodeVisibility=ordinaryCodeItem?.length?{nodeId:ordinaryCodeNode.id,opacity:Number(ordinaryCodeItem.style('opacity')),backgroundOpacity:Number(ordinaryCodeItem.style('background-opacity')),renderedWidth:Number.parseFloat(ordinaryCodeItem.renderedStyle('width'))}:{nodeId:'',opacity:0,backgroundOpacity:0,renderedWidth:0},visibleCodeNodes=state.cy?state.cy.nodes('[category = "code"]:visible'):[],communityColorCount=new Set(visibleCodeNodes.map(item=>item.data('color'))).size,defaultLabelCount=state.cy?state.cy.nodes(':visible').filter(item=>item.data('labelMode')==='always').length:0;return {canvasCount:layers.length,totalOpaqueSampleCount:layers.reduce((total,layer)=>total+layer.opaqueSampleCount,0),totalChromaticSampleCount:layers.reduce((total,layer)=>total+layer.chromaticSampleCount,0),communityColorCount,defaultLabelCount,ordinaryCodeVisibility,layers};}
    function runtimeProbeState(){const readout=document.getElementById('zoomReadout'),selectedRelation=state.selected?.type==='edge'?edgeById.get(state.selected.id):null,selectedEdgeItem=selectedRelation?state.cy.$id(selectedRelation.id):null;return {logicalZoom:Number(readout.textContent.replace('x','')),rendererZoom:Number(readout.dataset.rendererZoom||0),virtualGeometryScale:Number(readout.dataset.virtualGeometryScale||0),effectiveGeometryZoom:Number(readout.dataset.effectiveGeometryZoom||0),selectedEdgeRenderedWidth:Number(readout.dataset.selectedEdgeRenderedWidth||0),selectedEdgeRenderedOpacity:Number(readout.dataset.selectedEdgeRenderedOpacity||0),selectedEdgeLineStyle:selectedEdgeItem?.length?selectedEdgeItem.style('line-style'):'',selectedEdgeArrowShape:selectedEdgeItem?.length?selectedEdgeItem.style('target-arrow-shape'):'',selectedSourceRenderedWidth:Number(readout.dataset.selectedSourceRenderedWidth||0),selectedTargetRenderedWidth:Number(readout.dataset.selectedTargetRenderedWidth||0),selectedEdgeLinePixels:selectedRelation?selectedEdgeLinePixelEvidence(selectedRelation):{sampleCount:0,observedSampleCount:0,longestMissingRun:0,coverage:0,visibleSegmentLength:0}};}
    ...
    async function runRuntimeProbe(){
      const pause=milliseconds=>new Promise(resolve=>window.setTimeout(resolve,milliseconds)),frames=()=>new Promise(resolve=>window.requestAnimationFrame(()=>window.requestAnimationFrame(resolve))),endpointDistance=edge=>{const source=state.cy.$id(edge.source),target=state.cy.$id(edge.target);if(!source.length||!target.length)return 0;const a=source.position(),b=target.position();return Math.hypot(a.x-b.x,a.y-b.y);},nearestCenterDistance=()=>{const center={x:state.cy.width()/2,y:state.cy.height()/2};let distance=Infinity;state.cy.nodes(':visible').forEach(item=>{const point=item.renderedPosition();distance=Math.min(distance,Math.hypot(point.x-center.x,point.y-center.y));});return distance;};
      await pause(450);const selectedEdge=allEdges.find(edge=>edge.kind==='invokes-syntax')||allEdges.find(edge=>edge.category==='semantic-relation')||allEdges[0];if(!selectedEdge)throw new Error('runtime probe requires at least one relation');const overview={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeEndpointDistance:endpointDistance(selectedEdge)};
      document.getElementById('zoomMax').click();await frames();await pause(80);const unselectedMaximumAnchorDistance=nearestCenterDistance();document.getElementById('fitGraph').click();await frames();await pause(50);const hiddenAnchorNode=state.cy.nodes(':visible')[0],hiddenAnchorDisplay=hiddenAnchorNode?.style('display');if(hiddenAnchorNode?.length){state.selected={type:'node',id:hiddenAnchorNode.id()};hiddenAnchorNode.style('display','none');}document.getElementById('zoomMax').click();await frames();await pause(80);const hiddenSelectionMaximumAnchorDistance=nearestCenterDistance();if(hiddenAnchorNode?.length)hiddenAnchorNode.style('display',hiddenAnchorDisplay||'element');state.selected=null;document.getElementById('fitGraph').click();await frames();await pause(80);
      state.selected={type:'edge',id:selectedEdge.id};renderSelection();highlight();const hundredStarted=performance.now();document.getElementById('zoom100').click();await frames();const hundredInteractionMilliseconds=performance.now()-hundredStarted;await pause(90);updateZoomReadout();const hundred=runtimeProbeState(),nearMaximumStarted=performance.now();zoomGraphTo(511);await frames();const nearMaximumInteractionMilliseconds=performance.now()-nearMaximumStarted;await pause(100);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();await pause(60);const nearMaximum=runtimeProbeState(),maximumStarted=performance.now();document.getElementById('zoomMax').click();await frames();const maximumInteractionMilliseconds=performance.now()-maximumStarted;await pause(110);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();await pause(60);const panBefore={...state.cy.pan()},panStarted=performance.now();state.cy.pan({x:panBefore.x+18,y:panBefore.y-12});await frames();const panInteractionMilliseconds=performance.now()-panStarted,panAfter=state.cy.pan(),panDelta={x:panAfter.x-panBefore.x,y:panAfter.y-panBefore.y};state.cy.pan(panBefore);await frames();document.getElementById('zoomOut').click();await frames();await pause(70);updateZoomReadout();const afterZoomOut=runtimeProbeState();document.getElementById('zoomMax').click();await frames();await pause(100);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();await pause(60);
      const maximumState=runtimeProbeState(),maximumPixels=graphPixelEvidence(),selectedEdgeItem=state.cy.$id(selectedEdge.id);selectedEdgeItem.style('opacity',0);state.cy.forceRender();await frames();const hiddenLinePixels=selectedEdgeLinePixelEvidence(selectedEdge);selectedEdgeItem.removeStyle('opacity');refreshHighlightedEdgeScale();state.cy.forceRender();await frames();const maximum={state:maximumState,pixels:maximumPixels,pixelDetectorHiddenLineControl:hiddenLinePixels,selectedEdgeId:selectedEdge.id,selectedEdgeKind:selectedEdge.kind,selectedEdgeEndpointDistance:endpointDistance(selectedEdge),selectionText:document.getElementById('selectionInspector').innerText,navigation:{hundred,nearMaximum,afterZoomOut,panDelta,unselectedMaximumAnchorDistance,hiddenSelectionMaximumAnchorDistance,hundredInteractionMilliseconds,nearMaximumInteractionMilliseconds,maximumInteractionMilliseconds,panInteractionMilliseconds},graphifyPaletteApplied:state.cy.nodes('[category = "code"]').every(item=>communityPalette.includes(item.data('color'))),allVisibleNodeShapesDot:state.cy.nodes(':visible').every(item=>item.style('shape')==='ellipse'),materialOverlayPresent:Boolean(document.querySelector('.node-material-layer')),unsupportedConfidenceFallsBackToUnknown:normalizedEdgeConfidence('resolved-local-symbol')==='unknown'&&normalizedEdgeConfidence('unsupported-value')==='unknown'};maximum.endpointGeometryScale=overview.selectedEdgeEndpointDistance>0?maximum.selectedEdgeEndpointDistance/overview.selectedEdgeEndpointDistance:0;const checks=[
        ...
        {id:'overview-chromatic-communities-visible',passed:overview.pixels.totalChromaticSampleCount>=100&&overview.pixels.communityColorCount>=4,actual:{chromaticSamples:overview.pixels.totalChromaticSampleCount,communityColors:overview.pixels.communityColorCount},expected:'chromatic samples>=100 and community colors>=4'},
        {id:'overview-ordinary-code-visible',passed:Boolean(overview.pixels.ordinaryCodeVisibility.nodeId)&&overview.pixels.ordinaryCodeVisibility.opacity>=.99&&overview.pixels.ordinaryCodeVisibility.backgroundOpacity>=.99&&overview.pixels.ordinaryCodeVisibility.renderedWidth>=2,actual:overview.pixels.ordinaryCodeVisibility,expected:'ordinary code opacity/background>=.99 and rendered width>=2px'},
        {id:'graphify-categorical-palette-applied',passed:maximum.graphifyPaletteApplied,actual:maximum.graphifyPaletteApplied,expected:true},
        {id:'graphify-dot-node-shapes-applied',passed:maximum.allVisibleNodeShapesDot,actual:maximum.allVisibleNodeShapesDot,expected:true},
        {id:'hub-labels-sparse-by-default',passed:overview.pixels.defaultLabelCount>0&&overview.pixels.defaultLabelCount<Math.max(100,Math.ceil(allNodes.length*.05)),actual:overview.pixels.defaultLabelCount,expected:'1..'+(Math.max(100,Math.ceil(allNodes.length*.05))-1)},
        {id:'single-renderer-coordinate-space',passed:maximum.materialOverlayPresent===false,actual:maximum.materialOverlayPresent,expected:false},
        {id:'unsupported-confidence-fails-closed',passed:maximum.unsupportedConfidenceFallsBackToUnknown===true,actual:maximum.unsupportedConfidenceFallsBackToUnknown,expected:true},
        {id:'selected-invokes-syntax-edge-solid',passed:maximum.selectedEdgeKind==='invokes-syntax'&&maximum.state.selectedEdgeLineStyle==='solid',actual:{kind:maximum.selectedEdgeKind,lineStyle:maximum.state.selectedEdgeLineStyle},expected:'selected invokes-syntax relation rendered as solid'},
        {id:'selected-edge-arrow-disabled',passed:maximum.state.selectedEdgeArrowShape==='none',actual:maximum.state.selectedEdgeArrowShape,expected:'none'},
        {id:'selected-edge-line-pixel-continuous',passed:maximum.state.selectedEdgeLinePixels.sampleCount>=Math.min(2049,Math.ceil(maximum.state.selectedEdgeLinePixels.visibleSegmentLength)+1)&&maximum.state.selectedEdgeLinePixels.coverage>=.99&&maximum.state.selectedEdgeLinePixels.observedSampleCount<=maximum.state.selectedEdgeLinePixels.sampleCount&&maximum.state.selectedEdgeLinePixels.longestMissingRun<=1,actual:maximum.state.selectedEdgeLinePixels,expected:'whole visible relation sampled at <=1px intervals, coverage>=.99, longest missing run<=1'},
        {id:'edge-pixel-probe-detects-hidden-line',passed:maximum.pixelDetectorHiddenLineControl.sampleCount>=25&&maximum.pixelDetectorHiddenLineControl.coverage<=.10,actual:maximum.pixelDetectorHiddenLineControl,expected:'temporarily hidden selected line produces <=.10 coverage before style restoration'},
        {id:'zoom-100-control',passed:Math.abs(maximum.navigation.hundred.logicalZoom-100)<.001&&Math.abs(maximum.navigation.hundred.rendererZoom-100)<.001,actual:maximum.navigation.hundred,expected:'logical and renderer zoom 100'},
        {id:'zoom-maximum-control',passed:Math.abs(maximum.state.logicalZoom-512)<.001&&Math.abs(maximum.state.rendererZoom-512)<.001,actual:maximum.state,expected:'logical and renderer zoom 512'},
        {id:'unselected-deep-zoom-anchors-visible-node',passed:maximum.navigation.unselectedMaximumAnchorDistance<=1.5,actual:maximum.navigation.unselectedMaximumAnchorDistance,expected:'<=1.5 screen pixels'},
        {id:'hidden-selection-deep-zoom-anchors-visible-node',passed:maximum.navigation.hiddenSelectionMaximumAnchorDistance<=1.5,actual:maximum.navigation.hiddenSelectionMaximumAnchorDistance,expected:'<=1.5 screen pixels'},
        {id:'zoom-out-control',passed:maximum.navigation.afterZoomOut.logicalZoom<512&&maximum.navigation.afterZoomOut.logicalZoom>100,actual:maximum.navigation.afterZoomOut.logicalZoom,expected:'between 100 and 512'},
        {id:'maximum-pan-control',passed:Math.abs(maximum.navigation.panDelta.x-18)<.001&&Math.abs(maximum.navigation.panDelta.y+12)<.001,actual:maximum.navigation.panDelta,expected:{x:18,y:-12}},
        {id:'near-maximum-node-size-stable',passed:Math.abs(maximum.navigation.nearMaximum.logicalZoom-511)<.001&&[['selectedSourceRenderedWidth'],['selectedTargetRenderedWidth']].every(([key])=>maximum.navigation.nearMaximum[key]>=6&&maximum.navigation.nearMaximum[key]<=24.1&&Math.abs(maximum.navigation.nearMaximum[key]-maximum.state[key])<=1),actual:{nearMaximum:maximum.navigation.nearMaximum,maximum:maximum.state},expected:'511x and 512x endpoint widths remain bounded and differ by <=1px'},
        {id:'deep-navigation-render-settled',passed:[maximum.navigation.hundredInteractionMilliseconds,maximum.navigation.nearMaximumInteractionMilliseconds,maximum.navigation.maximumInteractionMilliseconds,maximum.navigation.panInteractionMilliseconds].every(value=>Number.isFinite(value)&&value>=0),actual:maximum.navigation,expected:'finite non-negative virtual-time diagnostics; wall-clock capture is bounded externally'},
        {id:'logical-zoom-512',passed:Math.abs(maximum.state.logicalZoom-512)<.001,actual:maximum.state.logicalZoom,expected:512},
        {id:'renderer-zoom-512',passed:Math.abs(maximum.state.rendererZoom-512)<.001,actual:maximum.state.rendererZoom,expected:512},
        {id:'effective-geometry-zoom-512',passed:Math.abs(maximum.state.effectiveGeometryZoom-512)<.001,actual:maximum.state.effectiveGeometryZoom,expected:512},
        {id:'virtual-geometry-scale-unity',passed:Math.abs(maximum.state.virtualGeometryScale-1)<.001,actual:maximum.state.virtualGeometryScale,expected:1},
        ...
        {id:'model-geometry-stable-at-actual-zoom',passed:overview.selectedEdgeEndpointDistance>0&&Math.abs(maximum.endpointGeometryScale-1)<.01,actual:maximum.endpointGeometryScale,expected:1},
        {id:'selected-edge-screen-width',passed:Math.abs(maximum.state.selectedEdgeRenderedWidth-2)<.05,actual:maximum.state.selectedEdgeRenderedWidth,expected:2},
        {id:'selected-edge-opacity',passed:Math.abs(maximum.state.selectedEdgeRenderedOpacity-.90)<.01,actual:maximum.state.selectedEdgeRenderedOpacity,expected:.90},
        {id:'selected-endpoint-node-size-bounded',passed:[maximum.state.selectedSourceRenderedWidth,maximum.state.selectedTargetRenderedWidth].every(value=>value>=6&&value<=24.1),actual:{source:maximum.state.selectedSourceRenderedWidth,target:maximum.state.selectedTargetRenderedWidth},expected:'6..24.1 screen pixels'},
...
def validate_projection(projection: dict[str, Any]) -> list[str]:
    ...
    required_true_ui_contract = (
        "fullGraphDefault", "allNodesLoaded", "allEdgesLoaded", "progressiveDetail",
        "viewportScaleCompensation", "relationAwareCommunityLayout", "zoomStyleBuckets",
        "actualCameraMaximumZoom", "browserRuntimeProbe", "headlessBrowserRegression",
        "canvasPixelEvidence", "graphifyCommunityDotRendering", "graphifyCategoricalPalette",
        "degreeWeightedNodeSizing", "hubOnlyDefaultLabels", "confidenceStyledRelations",
        "singleRendererCoordinateSpace", "continuousEdgesDuringViewportInteraction",
        "precisionDeepZoomBands", "supportingNodeLabelsOnDemand", "workStageTimeline",
        "durableWorkStageRevisions", "allRecordedWorkItemsNavigable", "workHistorySearch",
        "workHistoryStatusFilter", "boundedWorkHistoryRendering", "previousNextWorkNavigation",
        "previousNextStageNavigation", "liveProjectionRefreshAfterMutation",
        "staticRevisionSnapshotImmutable", "loopbackProjectStateMutationFromUi",
        "loopbackReviewProposalIntakeFromUi", "loopbackGuidedReviewProposalFromUi",
        "loopbackReviewReceiptIntakeFromUi", "loopbackGuidedReviewReceiptFromUi",
        "loopbackVerifierResultIntakeFromUi", "loopbackEvidenceDecisionFromUi",
        "clientSideEvidenceArtifactHashing", "externalEvidenceAcceptanceByWorkbench",
    )
    required_false_ui_contract = (
        "stellarVitreousNodeMaterial", "celestialCeramicNodeMaterial",
        "spectralTitaniumNodeMaterial", "astralForgedGlassNodeMaterial",
        "spectralObsidianNodeMaterial", "iridescentVoidNodeMaterial",
        "nebulaBlackMetalNodeMaterial", "luminousAlloyHierarchyContrast",
        "rendererSafeOpticalMaterial", "cachedCanvasNodeMaterial",
        "boundedMaterialSpriteCache", "viewportLocalMaterialRendering",
        "viewportSpatialMaterialIndex", "farZoomMaterialCulling",
        "selectedMaterialPixelEvidence", "selectedEdgeScreenSpaceTaper",
        "virtualPrecisionZoom", "staticGraphMutationFromUi", "targetRepositoryMutationFromUi",
        "physicsLayoutOnLoad", "externalVerifierExecutionByWorkbench",
        "reviewerAuthenticationByWorkbench",
    )
    if not isinstance(ui_contract, dict) or any(ui_contract.get(key) is not True for key in required_true_ui_contract) or any(ui_contract.get(key) is not False for key in required_false_ui_contract) or ui_contract.get("maximumZoom") != 512 or ui_contract.get("rendererMaximumZoom") != 512 or ui_contract.get("effectiveGeometryMaximumZoom") != 512 or ui_contract.get("virtualGeometryScaleAtMaximumZoom") != 1.0 or ui_contract.get("selectedEdgeRenderedWidthPixelsAtMaximumZoom") != 2.0 or ui_contract.get("selectedEdgeRenderedOpacityAtMaximumZoom") != 0.90 or ui_contract.get("overviewOrdinaryCodeOpacity") != 1.0 or ui_contract.get("overviewCodeRelationOpacity") != 0.70 or ui_contract.get("evidenceAcceptanceAuthorityScope") != "local-project-workspace":
...
def validate_output(output: Path, project_workspace: Path, expected_projection: dict[str, Any], project_before: list[dict[str, str]]) -> dict[str, Any]:
    ...
    for marker in ["id=\"projectGraph\"", "id=\"modeBadge\"", "id=\"workList\"", "id=\"previousWork\"", "id=\"nextWork\"", "id=\"workPosition\"", "id=\"stageList\"", "id=\"previousStage\"", "id=\"nextStage\"", "id=\"stagePosition\"", "id=\"selectionInspector\"", "id=\"changePanel\"", "id=\"deltaList\"", "id=\"evidencePanel\"", "id=\"authorityPanel\"", "id=\"zoomIn\"", "id=\"zoomOut\"", "id=\"zoom100\"", "id=\"zoom256\"", "id=\"zoomMax\"", "id=\"zoomReadout\"", "maxZoom:rendererMaximumZoom", "logicalZoomFromActual", "applyVirtualGeometry", "effectiveGeometryZoom", "selectedEdgeRenderedOpacity", "selectedSourceRenderedWidth", "selectedEdgeLinePixelEvidence", "selectedEdgeLinePixels", "communityPalette", "hubLabelThreshold", "normalizedEdgeConfidence", "unsupportedConfidenceFallsBackToUnknown", "edge[confidence = \"unknown\"]", "target-arrow-shape':'none'", "zoomAnchorModelPosition", "intentGraphRuntimeProbe", "intentgraph-runtime-probe-report", "graphPixelEvidence", "deepZoomFloors", "precision-", "spectralObsidianOpticalMaterial", "edge:selected", "assets/cytoscape.min.js"]:
