...
def run(snapshot: Path, output: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="p9.15-server-smoke-") as temporary:
        ...
        try:
            ...
            probes.append({"id": "serves-effective-100x-and-cached-optical-material", "passed": all(marker in html for marker in (b"applyVirtualGeometry", b"effectiveGeometryZoom", b"virtualGeometryScale", b"selectedEdgeRenderedOpacity", b"culledOrdinaryCode", b"node-material-layer", b"materialSprite", b"cached-spectral-titanium-v2", b"intentGraphRuntimeProbe", b"intentgraph-runtime-probe-report", b"graphPixelEvidence", b"scheduleMaterialLayer"))})
            ...
            probes.append({"id": "projects-effective-100x-rendering-contract", "passed": initial_projection["uiContract"]["effectiveGeometryMaximumZoom"] == 100 and initial_projection["uiContract"]["virtualGeometryScaleAtMaximumZoom"] == 4.1667 and initial_projection["uiContract"]["selectedEdgeRenderedWidthPixelsAtMaximumZoom"] == 0.065 and initial_projection["uiContract"]["selectedEdgeRenderedOpacityAtMaximumZoom"] == 0.34 and initial_projection["uiContract"]["spectralTitaniumNodeMaterial"] is True and initial_projection["uiContract"]["browserRuntimeProbe"] is True and initial_projection["uiContract"]["headlessBrowserRegression"] is True and initial_projection["uiContract"]["canvasPixelEvidence"] is True and initial_projection["uiContract"]["cachedCanvasNodeMaterial"] is True and initial_projection["uiContract"]["viewportLocalMaterialRendering"] is True and initial_projection["uiContract"]["farZoomMaterialCulling"] is True})
