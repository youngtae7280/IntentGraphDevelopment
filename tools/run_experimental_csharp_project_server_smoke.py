...
def run(snapshot: Path, output: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="p9.15-server-smoke-") as temporary:
        ...
        try:
            ...
            probes.append({"id": "serves-project-projection", "passed": status == 200 and head_status == 200 and initial_head["revisionCount"] == 0 and initial_head["workItemCount"] == 0 and initial_head["latestRevisionId"] is None and initial_head["projectStateVersion"].startswith("sha256:") and initial_projection["workflow"]["workItems"] == [] and initial_projection["workflow"]["workStageTimeline"] == [] and initial_projection["workflow"]["workStageRevisions"] == [] and initial_projection["workflow"]["timelineContract"]["durableRevisionCount"] == 0 and initial_projection["uiContract"]["allRecordedWorkItemsNavigable"] is True and initial_projection["uiContract"]["previousNextWorkNavigation"] is True and initial_projection["uiContract"]["previousNextStageNavigation"] is True and initial_projection["uiContract"]["liveProjectionRefreshAfterMutation"] is True and initial_projection["snapshot"]["semanticRelationOverlay"]["resolvedRelationCount"] == 1 and initial_projection["graph"]["relationCounts"].get("calls") == 1 and initial_projection["graph"]["defaultView"]["id"] == "all" and set(initial_projection["graph"]["views"]["all"]["nodeIds"]) == {node["id"] for node in initial_projection["graph"]["nodes"]}})
