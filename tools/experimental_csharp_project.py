...
def build_projection(project_workspace: Path) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    projection = {
        ...
        "uiContract": {
            ...
            "maximumZoom": 240,
            "selectedEdgeRenderedWidthPixels": 0.95,
...
<head>
  ...
  <style>
    :root { --left:286px; --right:356px; --void:#050710; --panel:#0c101c; --panel2:#0e1320; --line:#273149; --text:#edf4ff; --muted:#91a0b7; --accent:#32d5e6; --pink:#ff4fa3; --violet:#a855f7; --warn:#f5b82e; --danger:#ff4d67; --code:#38bdf8; --intent:#ffb000; --work:#12d6a0; --evidence:#9b5cff; --history:#6fa8ff; --proposal:#ff4d67; }
...
<body>
  ...
  <script>
    ...
    const state = { mode:model.graph.defaultView?.id || 'all', selected:null, selectedWorkId:workItems.length?workItems[workItems.length-1].id:null, stageFocus:null, cy:null, renderTimer:null, workFilterTimer:null, zoomSettleTimer:null, detailLevel:null, viewportScale:null, positions:null, visibleNodeIds:null, visibleEdgeIds:null, highlighted:null, highlightedEdgeIds:new Set(), emphasizedNodeIds:new Set(), emphasizedEdgeIds:new Set(), stageAddedNodeIds:new Set(), stageChangedNodeIds:new Set(), stageEdgeIds:new Set(), searchMatchNodeIds:new Set(), onDemandLabelIds:new Set(), resizeQueued:false, graphInstanceCount:0, visibilityUpdates:0, renderedWorkCardCount:0 };
    const colors = { code:'#38bdf8', project:'#22d3ee', 'source-document':'#60a5fa', goal:'#f5b82e', capability:'#12d6a0', constraint:'#f97316', 'verification-requirement':'#a855f7', work:'#10d89c', intent:'#ff4fa3', mapping:'#eab308', proposal:'#ff4d67', verification:'#8b5cf6', evidence:'#a855f7', 'review-receipt':'#c026ff', authority:'#3b82f6', history:'#647cff' };
    const communityPalette = ['#0ea5e9','#f59e0b','#f43f5e','#14b8a6','#22c55e','#eab308','#a855f7','#ec4899','#f97316','#8b9db8'];
    ...
    function makeSpectralObsidianNodeMaterialTexture(){
      ...
      const depth=context.createRadialGradient(37,27,2,68,70,86);depth.addColorStop(0,'rgba(220,249,255,.11)');depth.addColorStop(.2,'rgba(77,116,145,.035)');depth.addColorStop(.55,'rgba(2,7,16,.22)');depth.addColorStop(1,'rgba(0,2,8,.9)');context.fillStyle=depth;context.fillRect(0,0,128,128);
      const refraction=context.createLinearGradient(8,112,118,12);refraction.addColorStop(0,'rgba(37,226,220,0)');refraction.addColorStop(.37,'rgba(37,226,220,.055)');refraction.addColorStop(.5,'rgba(220,244,255,.14)');refraction.addColorStop(.58,'rgba(225,72,170,.045)');refraction.addColorStop(1,'rgba(225,72,170,0)');context.fillStyle=refraction;context.fillRect(0,0,128,128);
      context.beginPath();context.arc(64,64,54,Math.PI*1.08,Math.PI*1.78);context.strokeStyle='rgba(220,249,255,.36)';context.lineWidth=1.2;context.stroke();
      context.beginPath();context.arc(64,64,52,Math.PI*.05,Math.PI*.68);context.strokeStyle='rgba(28,224,215,.14)';context.lineWidth=.8;context.stroke();
      context.beginPath();context.moveTo(30,92);context.lineTo(97,29);context.strokeStyle='rgba(255,255,255,.055)';context.lineWidth=.7;context.stroke();
      let seed=9173;for(let index=0;index<14;index+=1){seed=(seed*48271)%2147483647;const x=18+(seed%92);seed=(seed*48271)%2147483647;const y=18+(seed%92);seed=(seed*48271)%2147483647;const alpha=.018+(seed%7)/1000;context.fillStyle=`rgba(205,242,255,${alpha})`;context.fillRect(x,y,1,1);}
      return canvas.toDataURL('image/png');
    }
    const spectralObsidianNodeMaterialTexture=makeSpectralObsidianNodeMaterialTexture();
    function graphStyle(){return [
      {selector:'node',style:{'shape':'ellipse','background-color':'data(color)','background-opacity':.78,'background-blacken':.86,'background-image':'none','label':'data(label)','color':'#f0f7ff','font-family':'Inter,Segoe UI,Arial,sans-serif','font-weight':600,'font-size':9.5,'text-wrap':'ellipsis','text-max-width':126,'text-valign':'bottom','text-margin-y':5,'width':15,'height':15,'border-width':.8,'border-color':'data(color)','border-opacity':.88,'underlay-color':'data(color)','underlay-opacity':.04,'underlay-padding':1.6,'underlay-shape':'ellipse','overlay-opacity':0,'text-outline-width':1.5,'text-outline-color':'#050812','text-outline-opacity':.94}},
      {selector:'node[category = "code"]',style:{'label':'','background-color':'data(color)','background-opacity':.96,'background-blacken':.14,'background-image':'none','width':2.6,'height':2.6,'opacity':1,'border-width':.35,'border-color':'#d8ecff','border-opacity':.24,'underlay-opacity':0}},
      {selector:'node[category = "code"][kind = "file"]',style:{'shape':'round-rectangle','width':6.5,'height':6.5,'background-opacity':1,'background-blacken':.42,'background-image':spectralObsidianNodeMaterialTexture,'background-image-opacity':.48,'border-width':.55,'border-color':'data(color)','border-opacity':.9,'underlay-color':'data(color)','underlay-opacity':.035,'underlay-padding':1.25}},
      {selector:'node[category = "code"][kind = "namespace"]',style:{'shape':'diamond','width':4.3,'height':4.3}},
      {selector:'node[category = "code"][kind = "type"]',style:{'shape':'round-hexagon','width':5.2,'height':5.2}},
      ...
      {selector:'node[category = "code-capsule"]',style:{'background-color':'data(color)','background-opacity':.7,'background-blacken':.9,'background-image':'none','shape':'round-hexagon','border-width':.85,'border-color':'data(color)','border-opacity':.9,'underlay-color':'data(color)','underlay-opacity':.03,'underlay-padding':1.35,'font-size':9,'font-weight':650,'text-max-width':135,'width':23,'height':23,'color':'#e4f3ff'}},
      ...
      {selector:'node:selected',style:{'border-width':1.8,'border-color':'#d8fbff','border-opacity':1,'underlay-color':'#61efff','underlay-opacity':.14,'underlay-padding':2.2,'overlay-opacity':0,'z-index':99}},
      {selector:'edge:selected',style:{'line-color':'#c8f6ff','opacity':.98,'overlay-opacity':0,'underlay-opacity':0,'z-index':99}},
      {selector:'edge.selection-neighbor',style:{'line-color':'#8de0e8','width':.75,'opacity':.82,'overlay-opacity':0,'underlay-opacity':0,'z-index':98}}
    ];}
    ...
    function renderGraph(options={}) {
      ...
      if(!state.cy){
        ...
        state.cy=cytoscape({
          ...
          minZoom:.10,maxZoom:240,pixelRatio:1,textureOnViewport:true,motionBlur:true,hideEdgesOnViewport:true,boxSelectionEnabled:false,autoungrabify:true
        });
        ...
        state.cy.on('zoom',()=>{updateZoomReadout();refreshHighlightedEdgeScale();scheduleSettledSemanticZoom();});
      }
    ...
    function refreshHighlightedEdgeScale(){if(!state.cy||!state.highlightedEdgeIds.size)return;const zoom=Math.max(.1,state.cy.zoom());state.highlightedEdgeIds.forEach(id=>{const selected=state.selected?.type==='edge'&&state.selected.id===id;state.cy.$id(id).style('width',(selected ? 0.95 : 0.68)/zoom);});}
    function updateViewportScale() {
      if(!state.cy)return;
      const zoom=state.cy.zoom(),band=zoom<.25?'far':zoom<.52?'overview':zoom<1.05?'structure':zoom<1.8?'detail':zoom<3.5?'close':zoom<7?'deep':zoom<18?'inspect':zoom<45?'ultra':zoom<90?'micro':zoom<160?'nano':'atomic';
      if(state.viewportScale===band){refreshHighlightedEdgeScale();return;}
      state.viewportScale=band;
      const profiles={far:{scale:7,node:18,code:2.6,file:5.8,namespace:3.8,type:4.7,capsule:20,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.34,codeEdge:.18,semanticEdge:.84},overview:{scale:3,node:18,code:2.8,file:6.2,namespace:4.1,type:5,capsule:21,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.4,codeEdge:.22,semanticEdge:.92},structure:{scale:1.4,node:19,code:3.5,file:7,namespace:4.8,type:5.8,capsule:22,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.48,codeEdge:.28,semanticEdge:1},detail:{scale:.75,node:20,code:5,file:8.4,namespace:6,type:7.1,capsule:24,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.54,codeEdge:.34,semanticEdge:1.08},close:{scale:.38,node:21,code:8,file:10,namespace:8.4,type:9.2,capsule:26,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.62,codeEdge:.46,semanticEdge:1.16},deep:{scale:.19,node:22,code:12,file:14,namespace:12.5,type:13.2,capsule:28,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.7,codeEdge:.62,semanticEdge:1.24},inspect:{scale:.085,node:23,code:17,file:20,namespace:18,type:19,capsule:30,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.78,codeEdge:.78,semanticEdge:1.34},ultra:{scale:.035,node:24,code:19,file:22,namespace:20,type:21,capsule:32,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.84,codeEdge:.84,semanticEdge:1.4},micro:{scale:.011,node:25,code:22,file:25,namespace:23,type:24,capsule:34,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.9,codeEdge:.9,semanticEdge:1.45},nano:{scale:.0058,node:25.5,code:22.5,file:25.5,namespace:23.5,type:24.5,capsule:34.5,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:.96,codeEdge:.96,semanticEdge:1.5},atomic:{scale:.0032,node:26,code:23,file:26,namespace:24,type:25,capsule:35,label:9.5,onDemandLabel:8.5,border:.8,codeBorder:.32,glow:1.6,edge:1.02,codeEdge:1.02,semanticEdge:1.55}};
      const profile=profiles[band],size=value=>Math.max(.001,Math.round(value*profile.scale*1000)/1000),apply=(selector,style)=>state.cy.$(selector).style(style);
      // Full collections are restyled only when crossing one of eleven zoom bands, not for every wheel tick.
      state.cy.batch(()=>{
        apply('node',{'width':size(profile.node),'height':size(profile.node),'font-size':size(profile.label),'border-width':size(profile.border),'underlay-padding':size(profile.glow),'text-margin-y':size(5),'text-outline-width':size(1.5)});apply('node.show-on-demand-label',{'font-size':size(profile.onDemandLabel)});apply('node[category = "code"]',{'width':size(profile.code),'height':size(profile.code),'border-width':size(profile.codeBorder)});apply('node[category = "code"][kind = "file"]',{'width':size(profile.file),'height':size(profile.file),'border-width':size(profile.codeBorder*1.7),'underlay-padding':size(1.25),'background-image':spectralObsidianNodeMaterialTexture,'background-image-opacity':.48});apply('node[category = "code"][kind = "namespace"]',{'width':size(profile.namespace),'height':size(profile.namespace)});apply('node[category = "code"][kind = "type"]',{'width':size(profile.type),'height':size(profile.type)});apply('node[category = "code-capsule"]',{'width':size(profile.capsule),'height':size(profile.capsule),'border-width':size(profile.border),'underlay-padding':size(1.8)});apply('edge',{'width':size(profile.edge)});apply('edge[category = "code-relation"]',{'width':size(profile.codeEdge)});apply('edge.detail-edge',{'width':size(profile.codeEdge*1.3)});apply('edge[category = "capsule-relation"]',{'width':size(profile.semanticEdge*.82)});apply('edge[category = "mapping-relation"]',{'width':size(profile.semanticEdge)});apply('edge[category = "delta-relation"]',{'width':size(profile.semanticEdge)});apply('edge.selection-neighbor',{'width':size(profile.semanticEdge)});apply('edge.semantic-emphasis',{'width':size(profile.semanticEdge)});
      });
...
def validate_projection(projection: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(ui_contract, dict) or any(ui_contract.get(key) is not True for key in ("fullGraphDefault", "allNodesLoaded", "allEdgesLoaded", "progressiveDetail", "viewportScaleCompensation", "relationAwareCommunityLayout", "zoomStyleBuckets", "spectralObsidianNodeMaterial", "supportingNodeLabelsOnDemand", "workStageTimeline", "durableWorkStageRevisions", "allRecordedWorkItemsNavigable", "workHistorySearch", "workHistoryStatusFilter", "boundedWorkHistoryRendering", "previousNextWorkNavigation", "previousNextStageNavigation", "liveProjectionRefreshAfterMutation", "staticRevisionSnapshotImmutable", "loopbackProjectStateMutationFromUi", "loopbackReviewProposalIntakeFromUi", "loopbackGuidedReviewProposalFromUi", "loopbackReviewReceiptIntakeFromUi", "loopbackGuidedReviewReceiptFromUi")) or ui_contract.get("maximumZoom") != 240 or ui_contract.get("selectedEdgeRenderedWidthPixels") != 0.95 or any(ui_contract.get(key) is not False for key in ("staticGraphMutationFromUi", "targetRepositoryMutationFromUi", "physicsLayoutOnLoad")):
...
def validate_output(output: Path, project_workspace: Path, expected_projection: dict[str, Any], project_before: list[dict[str, str]]) -> dict[str, Any]:
    ...
    for marker in ["id=\"projectGraph\"", "id=\"modeBadge\"", "id=\"workList\"", "id=\"previousWork\"", "id=\"nextWork\"", "id=\"workPosition\"", "id=\"stageList\"", "id=\"previousStage\"", "id=\"nextStage\"", "id=\"stagePosition\"", "id=\"selectionInspector\"", "id=\"changePanel\"", "id=\"deltaList\"", "id=\"evidencePanel\"", "id=\"authorityPanel\"", "id=\"zoomIn\"", "id=\"zoomOut\"", "id=\"zoomReadout\"", "maxZoom:240", "?'nano':'atomic'", "makeSpectralObsidianNodeMaterialTexture", "edge:selected", "assets/cytoscape.min.js"]:
