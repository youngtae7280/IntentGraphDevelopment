...
def build_projection(project_workspace: Path) -> tuple[dict[str, Any], list[dict[str, str]]]:
    ...
    projection = {
        ...
        "uiContract": {
            ...
            "selectedEdgeRenderedWidthPixelsAtMaximumZoom": 0.30,
            "selectedEdgeRenderedOpacityAtMaximumZoom": 0.42,
            ...
            "celestialCeramicNodeMaterial": True,
...
<body>
  ...
  <script>
    ...
    function materialSprite(node,selected,accent){const color=nodeColor(node),shape=materialShape(node),key=['celestial-ceramic-v4',color,shape,selected?'selected':'normal',accent||''].join('|');if(state.materialSpriteCache.has(key))return state.materialSpriteCache.get(key);const canvas=document.createElement('canvas');canvas.width=canvas.height=192;const context=canvas.getContext('2d'),cx=96,cy=96,radius=42;
      // Cached black-ceramic optics: restrained bloom, asymmetric spectral rim, and etched depth without plastic gradients.
      const halo=context.createRadialGradient(cx-4,cy-7,radius*.78,cx,cy,72);halo.addColorStop(0,rgba(color,selected?.10:.035));halo.addColorStop(.54,rgba(color,selected?.025:.009));halo.addColorStop(1,rgba(color,0));context.fillStyle=halo;context.fillRect(0,0,192,192);
      context.save();context.shadowColor='rgba(0,1,5,.98)';context.shadowBlur=14;context.shadowOffsetY=5;traceMaterialShape(context,shape,cx,cy,radius+2.2);context.fillStyle='rgba(0,2,7,.99)';context.fill();context.restore();
      context.save();traceMaterialShape(context,shape,cx,cy,radius);context.clip();const core=context.createRadialGradient(78,69,3,99,100,61);core.addColorStop(0,'rgba(34,48,61,.98)');core.addColorStop(.24,'rgba(10,17,27,.99)');core.addColorStop(.7,'rgba(2,5,11,1)');core.addColorStop(1,'rgba(0,2,7,1)');context.fillStyle=core;context.fillRect(48,48,96,96);
      const anisotropic=context.createLinearGradient(58,139,139,51);anisotropic.addColorStop(0,'rgba(0,0,0,.42)');anisotropic.addColorStop(.38,'rgba(0,0,0,0)');anisotropic.addColorStop(.475,rgba(color,.11));anisotropic.addColorStop(.5,'rgba(184,238,241,.25)');anisotropic.addColorStop(.516,'rgba(190,111,174,.11)');anisotropic.addColorStop(.58,'rgba(0,0,0,0)');anisotropic.addColorStop(1,'rgba(0,0,0,.56)');context.fillStyle=anisotropic;context.fillRect(47,47,98,98);
      const lowerShade=context.createLinearGradient(61,58,131,136);lowerShade.addColorStop(0,'rgba(232,250,250,.045)');lowerShade.addColorStop(.34,'rgba(0,0,0,0)');lowerShade.addColorStop(.72,'rgba(0,0,0,.31)');lowerShade.addColorStop(1,'rgba(0,0,0,.74)');context.fillStyle=lowerShade;context.fillRect(47,47,98,98);
      context.globalCompositeOperation='screen';context.beginPath();context.moveTo(58,119);context.lineTo(89,61);context.lineTo(132,71);context.strokeStyle=rgba(color,.13);context.lineWidth=.72;context.stroke();context.beginPath();context.moveTo(65,127);context.lineTo(108,132);context.lineTo(137,84);context.strokeStyle='rgba(171,229,233,.075)';context.lineWidth=.55;context.stroke();context.globalCompositeOperation='source-over';
      let grainSeed=stableHash(key);for(let grainIndex=0;grainIndex<34;grainIndex+=1){grainSeed=Math.imul(grainSeed^grainIndex,1664525)+1013904223>>>0;const gy=58+(grainSeed%7600)/100,gx=58+((grainSeed>>>11)%7600)/100,bright=grainIndex%9===0;context.fillStyle=bright?'rgba(215,242,244,.18)':rgba(color,.045);context.fillRect(gx,gy,bright?1.15:.55,bright?.72:.38);}context.restore();
      traceMaterialShape(context,shape,cx,cy,radius+.7);context.strokeStyle='rgba(1,4,10,.98)';context.lineWidth=4.2;context.stroke();traceMaterialShape(context,shape,cx,cy,radius+.28);context.strokeStyle=rgba(color,selected?.88:.58);context.lineWidth=1.38;context.stroke();
      context.save();context.globalCompositeOperation='screen';traceMaterialShape(context,shape,cx,cy,radius-.95);context.strokeStyle='rgba(205,242,244,.28)';context.lineWidth=.72;context.stroke();context.beginPath();context.arc(cx,cy,radius+.45,-2.52,-.56);context.strokeStyle='rgba(218,250,250,.72)';context.lineWidth=.88;context.stroke();context.beginPath();context.arc(cx,cy,radius+.15,.30,1.58);context.strokeStyle='rgba(179,90,165,.28)';context.lineWidth=.82;context.stroke();context.restore();
      context.save();traceMaterialShape(context,shape,cx,cy,radius+.2);context.clip();context.beginPath();context.moveTo(62,72);context.lineTo(87,54);context.lineTo(118,59);context.strokeStyle='rgba(229,252,251,.34)';context.lineWidth=.82;context.stroke();context.beginPath();context.moveTo(132,108);context.lineTo(118,137);context.strokeStyle=rgba(color,.33);context.lineWidth=.68;context.stroke();context.restore();
      if(accent){context.save();context.globalCompositeOperation='screen';context.beginPath();context.arc(cx,cy,radius+2.1,-2.35,-.72);context.strokeStyle=accent;context.globalAlpha=.46;context.lineWidth=.82;context.stroke();context.restore();}if(selected){context.save();context.globalCompositeOperation='screen';context.beginPath();context.arc(cx,cy,radius+1.65,-2.72,-.18);context.strokeStyle='rgba(188,241,243,.62)';context.lineWidth=.78;context.stroke();context.fillStyle='rgba(228,253,252,.82)';for(const point of [[65,69],[129,125]])context.fillRect(point[0],point[1],1.25,1.25);context.restore();}
    ...
    function highlightedEdgeScreenWidth(zoom,selected){if(zoom>=160)return selected ? .30 : .07;if(zoom>=100)return selected ? .36 : .08;if(zoom>=80)return selected ? .40 : .09;if(zoom>=45)return selected ? .48 : .11;if(zoom>=18)return selected ? .58 : .14;return selected ? .82 : .22;}
    function highlightedEdgeOpacity(zoom,selected){if(zoom>=160)return selected ? .42 : .075;if(zoom>=100)return selected ? .46 : .085;if(zoom>=80)return selected ? .49 : .095;if(zoom>=45)return selected ? .55 : .11;if(zoom>=18)return selected ? .62 : .14;return selected ? .70 : .18;}
    ...
    async function runRuntimeProbe(){const pause=milliseconds=>new Promise(resolve=>window.setTimeout(resolve,milliseconds)),frames=()=>new Promise(resolve=>window.requestAnimationFrame(()=>window.requestAnimationFrame(resolve))),endpointDistance=edge=>{const source=state.cy.$id(edge.source),target=state.cy.$id(edge.target);if(!source.length||!target.length)return 0;const a=source.position(),b=target.position();return Math.hypot(a.x-b.x,a.y-b.y);};await pause(450);drawMaterialLayer();await pause(80);const selectedEdge=allEdges.find(edge=>edge.category==='semantic-relation')||allEdges[0];if(!selectedEdge)throw new Error('runtime probe requires at least one relation');const overview={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeEndpointDistance:endpointDistance(selectedEdge)};state.selected={type:'edge',id:selectedEdge.id};renderSelection();highlight();const hundredStarted=performance.now();document.getElementById('zoom100').click();const hundredHandlerMilliseconds=performance.now()-hundredStarted;await frames();await pause(90);updateZoomReadout();const hundred=runtimeProbeState(),maximumStarted=performance.now();document.getElementById('zoomMax').click();const maximumHandlerMilliseconds=performance.now()-maximumStarted;await frames();await pause(110);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();drawMaterialLayer();await pause(60);const panBefore={...state.cy.pan()},panStarted=performance.now();state.cy.pan({x:panBefore.x+18,y:panBefore.y-12});const panHandlerMilliseconds=performance.now()-panStarted;await frames();const panAfter=state.cy.pan(),panDelta={x:panAfter.x-panBefore.x,y:panAfter.y-panBefore.y};state.cy.pan(panBefore);await frames();document.getElementById('zoomOut').click();await frames();await pause(70);updateZoomReadout();const afterZoomOut=runtimeProbeState();document.getElementById('zoomMax').click();await frames();await pause(100);updateViewportScale();updateZoomReadout();refreshHighlightedEdgeScale();drawMaterialLayer();await pause(60);const maximum={state:runtimeProbeState(),pixels:graphPixelEvidence(),selectedEdgeId:selectedEdge.id,selectedEdgeEndpointDistance:endpointDistance(selectedEdge),selectedEndpointMaterialPixels:materialNodePixelEvidence(selectedEdge.source),selectionText:document.getElementById('selectionInspector').innerText,navigation:{hundred,afterZoomOut,panDelta,hundredHandlerMilliseconds,maximumHandlerMilliseconds,panHandlerMilliseconds}};maximum.endpointGeometryScale=overview.selectedEdgeEndpointDistance>0?maximum.selectedEdgeEndpointDistance/overview.selectedEdgeEndpointDistance:0;const checks=[
      ...
      {id:'celestial-ceramic-material-active',passed:maximum.state.materialProfile==='cached-celestial-ceramic-v4',actual:maximum.state.materialProfile,expected:'cached-celestial-ceramic-v4'},
      ...
      {id:'deep-navigation-handler-budget',passed:maximum.navigation.hundredHandlerMilliseconds<1000&&maximum.navigation.maximumHandlerMilliseconds<1000&&maximum.navigation.panHandlerMilliseconds<250,actual:maximum.navigation,expected:'100x/256x handlers <1000ms and pan <250ms'},
      ...
      {id:'selected-edge-screen-width',passed:Math.abs(maximum.state.selectedEdgeRenderedWidth-.30)<.002,actual:maximum.state.selectedEdgeRenderedWidth,expected:.30},
      {id:'selected-edge-opacity',passed:Math.abs(maximum.state.selectedEdgeRenderedOpacity-.42)<.002,actual:maximum.state.selectedEdgeRenderedOpacity,expected:.42},
