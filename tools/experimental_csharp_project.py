...
<body>
  ...
  <script>
    ...
    function makeNodeMaterialTexture(){
      const canvas=document.createElement('canvas');canvas.width=canvas.height=128;const context=canvas.getContext('2d');
      const depth=context.createRadialGradient(42,34,2,64,64,76);depth.addColorStop(0,'rgba(255,255,255,.3)');depth.addColorStop(.13,'rgba(178,231,255,.12)');depth.addColorStop(.38,'rgba(255,255,255,.018)');depth.addColorStop(.68,'rgba(1,7,17,.2)');depth.addColorStop(1,'rgba(0,2,9,.82)');context.fillStyle=depth;context.fillRect(0,0,128,128);
      const caustic=context.createLinearGradient(18,12,104,112);caustic.addColorStop(0,'rgba(255,255,255,0)');caustic.addColorStop(.36,'rgba(193,240,255,.11)');caustic.addColorStop(.46,'rgba(255,255,255,.02)');caustic.addColorStop(.72,'rgba(255,255,255,0)');context.fillStyle=caustic;context.fillRect(0,0,128,128);
      context.beginPath();context.arc(64,64,53,0,Math.PI*2);context.strokeStyle='rgba(211,246,255,.24)';context.lineWidth=2;context.stroke();context.beginPath();context.arc(64,64,48,Math.PI*1.05,Math.PI*1.76);context.strokeStyle='rgba(255,255,255,.13)';context.lineWidth=1;context.stroke();
      const glint=context.createRadialGradient(39,31,0,39,31,17);glint.addColorStop(0,'rgba(255,255,255,.86)');glint.addColorStop(.16,'rgba(211,247,255,.26)');glint.addColorStop(1,'rgba(255,255,255,0)');context.fillStyle=glint;context.fillRect(20,12,38,38);
      let seed=9173;for(let index=0;index<30;index+=1){seed=(seed*48271)%2147483647;const x=12+(seed%104);seed=(seed*48271)%2147483647;const y=12+(seed%104);seed=(seed*48271)%2147483647;const alpha=.035+(seed%8)/1000;context.fillStyle=`rgba(220,247,255,${alpha})`;context.fillRect(x,y,1,1);}
      return canvas.toDataURL('image/png');
    }
    const nodeMaterialTexture=makeNodeMaterialTexture();
    function graphStyle(){return [
      {selector:'node',style:{'shape':'ellipse','background-color':'data(color)','background-opacity':.98,'background-blacken':.69,'background-image':nodeMaterialTexture,'background-fit':'cover','background-clip':'node','background-image-opacity':.72,'label':'data(label)','color':'#f0f7ff','font-family':'Inter,Segoe UI,Arial,sans-serif','font-weight':600,'font-size':9.5,'text-wrap':'ellipsis','text-max-width':126,'text-valign':'bottom','text-margin-y':5,'width':15,'height':15,'border-width':1.15,'border-color':'data(color)','border-opacity':1,'underlay-color':'data(color)','underlay-opacity':.16,'underlay-padding':3.2,'underlay-shape':'ellipse','text-outline-width':1.5,'text-outline-color':'#050812','text-outline-opacity':.94}},
      ...
      {selector:'node[category = "code"][kind = "file"]',style:{'shape':'round-rectangle','width':6.5,'height':6.5,'background-opacity':1,'background-blacken':.2,'background-image':nodeMaterialTexture,'background-image-opacity':.55,'border-width':.55,'border-color':'data(color)','border-opacity':1,'underlay-color':'data(color)','underlay-opacity':.06,'underlay-padding':1.6}},
      ...
      {selector:'node[category = "code-capsule"]',style:{'background-color':'data(color)','background-opacity':.96,'background-blacken':.76,'background-image':nodeMaterialTexture,'background-image-opacity':.42,'shape':'ellipse','border-width':1.15,'border-color':'data(color)','border-opacity':1,'underlay-color':'data(color)','underlay-opacity':.1,'underlay-padding':2.2,'font-size':9,'font-weight':650,'text-max-width':135,'width':23,'height':23,'color':'#e4f3ff'}},
      ...
      {selector:':selected',style:{'border-width':2.4,'border-color':'#d8fbff','border-opacity':1,'underlay-color':'#61efff','underlay-opacity':.32,'line-color':'#aeeaf0','z-index':99}},
    ...
    function renderGraph(options={}) {
      ...
      if(!state.cy){
        ...
        state.cy=cytoscape({
          ...
          minZoom:.10,maxZoom:100,pixelRatio:1,textureOnViewport:true,motionBlur:true,hideEdgesOnViewport:true,boxSelectionEnabled:false,autoungrabify:true
        });
        ...
        state.cy.on('zoom',scheduleSettledSemanticZoom);
      }
    ...
    function refreshHighlightedEdgeScale(){if(!state.cy||!state.highlightedEdgeIds.size)return;const width=1.6/Math.max(.1,state.cy.zoom());state.highlightedEdgeIds.forEach(id=>state.cy.$id(id).style('width',width));}
    function updateViewportScale() {
      if(!state.cy)return;
      const zoom=state.cy.zoom(),band=zoom<.25?'far':zoom<.52?'overview':zoom<1.05?'structure':zoom<1.8?'detail':zoom<3.5?'close':zoom<7?'deep':zoom<18?'inspect':zoom<45?'ultra':'micro';
      if(state.viewportScale===band){refreshHighlightedEdgeScale();return;}
      state.viewportScale=band;
      const profiles={far:{scale:7,node:18,code:2.6,file:5.8,namespace:3.8,type:4.7,capsule:20,label:9.5,onDemandLabel:8.5,border:1.15,codeBorder:.32,glow:3.2,edge:.34,codeEdge:.18,semanticEdge:.84},overview:{scale:3,node:18,code:2.8,file:6.2,namespace:4.1,type:5,capsule:21,label:9.5,onDemandLabel:8.5,border:1.15,codeBorder:.32,glow:3.2,edge:.4,codeEdge:.22,semanticEdge:.92},structure:{scale:1.4,node:19,code:3.5,file:7,namespace:4.8,type:5.8,capsule:22,label:9.5,onDemandLabel:8.5,border:1.15,codeBorder:.32,glow:3.2,edge:.48,codeEdge:.28,semanticEdge:1},detail:{scale:.75,node:20,code:5,file:8.4,namespace:6,type:7.1,capsule:24,label:9.5,onDemandLabel:8.5,border:1.15,codeBorder:.32,glow:3.2,edge:.54,codeEdge:.34,semanticEdge:1.08},close:{scale:.38,node:21,code:8,file:10,namespace:8.4,type:9.2,capsule:26,label:9.5,onDemandLabel:8.5,border:1.15,codeBorder:.32,glow:3.2,edge:.62,codeEdge:.46,semanticEdge:1.16},deep:{scale:.19,node:22,code:12,file:14,namespace:12.5,type:13.2,capsule:28,label:9.5,onDemandLabel:8.5,border:1.15,codeBorder:.32,glow:3.2,edge:.7,codeEdge:.62,semanticEdge:1.24},inspect:{scale:.085,node:23,code:17,file:20,namespace:18,type:19,capsule:30,label:9.5,onDemandLabel:8.5,border:1.15,codeBorder:.32,glow:3.2,edge:.78,codeEdge:.78,semanticEdge:1.34},ultra:{scale:.035,node:24,code:19,file:22,namespace:20,type:21,capsule:32,label:9.5,onDemandLabel:8.5,border:1.15,codeBorder:.32,glow:3.2,edge:.84,codeEdge:.84,semanticEdge:1.4},micro:{scale:.011,node:25,code:22,file:25,namespace:23,type:24,capsule:34,label:9.5,onDemandLabel:8.5,border:1.15,codeBorder:.32,glow:3.2,edge:.9,codeEdge:.9,semanticEdge:1.45}};
      const profile=profiles[band],size=value=>Math.max(.005,Math.round(value*profile.scale*1000)/1000),apply=(selector,style)=>state.cy.$(selector).style(style);
      // Full collections are restyled only when crossing one of nine zoom bands, not for every wheel tick.
      state.cy.batch(()=>{
        apply('node',{'width':size(profile.node),'height':size(profile.node),'font-size':size(profile.label),'border-width':size(profile.border),'underlay-padding':size(profile.glow),'text-margin-y':size(5),'text-outline-width':size(1.5)});apply('node.show-on-demand-label',{'font-size':size(profile.onDemandLabel)});apply('node[category = "code"]',{'width':size(profile.code),'height':size(profile.code),'border-width':size(profile.codeBorder)});apply('node[category = "code"][kind = "file"]',{'width':size(profile.file),'height':size(profile.file),'border-width':size(profile.codeBorder*1.85),'underlay-padding':size(1.8),'background-image':nodeMaterialTexture,'background-image-opacity':.55});apply('node[category = "code"][kind = "namespace"]',{'width':size(profile.namespace),'height':size(profile.namespace)});apply('node[category = "code"][kind = "type"]',{'width':size(profile.type),'height':size(profile.type)});apply('node[category = "code-capsule"]',{'width':size(profile.capsule),'height':size(profile.capsule),'border-width':size(profile.border),'underlay-padding':size(2.6)});apply('edge',{'width':size(profile.edge)});apply('edge[category = "code-relation"]',{'width':size(profile.codeEdge)});apply('edge.detail-edge',{'width':size(profile.codeEdge*1.3)});apply('edge[category = "capsule-relation"]',{'width':size(profile.semanticEdge*.82)});apply('edge[category = "mapping-relation"]',{'width':size(profile.semanticEdge)});apply('edge[category = "delta-relation"]',{'width':size(profile.semanticEdge)});apply('edge.selection-neighbor',{'width':size(profile.semanticEdge)});apply('edge.semantic-emphasis',{'width':size(profile.semanticEdge)});
      });
    ...
    document.getElementById('previousWork').addEventListener('click',()=>moveWork(-1));document.getElementById('nextWork').addEventListener('click',()=>moveWork(1));document.getElementById('previousStage').addEventListener('click',()=>moveStage(-1));document.getElementById('nextStage').addEventListener('click',()=>moveStage(1));document.getElementById('zoomIn').addEventListener('click',()=>state.cy.zoom({level:state.cy.zoom()*1.7,renderedPosition:{x:state.cy.width()/2,y:state.cy.height()/2}}));document.getElementById('zoomOut').addEventListener('click',()=>state.cy.zoom({level:state.cy.zoom()/1.7,renderedPosition:{x:state.cy.width()/2,y:state.cy.height()/2}}));document.getElementById('fitGraph').addEventListener('click',()=>{const graph=visibleGraphData(),semanticFocus=state.mode==='overview'&&!filters().search&&!filters().category&&!filters().relation?model.graph.views.overview.nodeIds.map(id=>nodeById.get(id)).filter(Boolean):graph.nodes;fitNodes(semanticFocus);});init();staticPanels();renderGraph({fit:true});renderSelection();resize();window.dispatchEvent(new Event('intentgraph-ready'));
    }
...
def validate_projection(projection: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(ui_contract, dict) or any(ui_contract.get(key) is not True for key in ("fullGraphDefault", "allNodesLoaded", "allEdgesLoaded", "progressiveDetail", "viewportScaleCompensation", "relationAwareCommunityLayout", "zoomStyleBuckets", "supportingNodeLabelsOnDemand", "workStageTimeline", "durableWorkStageRevisions", "allRecordedWorkItemsNavigable", "workHistorySearch", "workHistoryStatusFilter", "boundedWorkHistoryRendering", "previousNextWorkNavigation", "previousNextStageNavigation", "liveProjectionRefreshAfterMutation", "staticRevisionSnapshotImmutable", "loopbackProjectStateMutationFromUi", "loopbackReviewProposalIntakeFromUi", "loopbackGuidedReviewProposalFromUi", "loopbackReviewReceiptIntakeFromUi", "loopbackGuidedReviewReceiptFromUi")) or any(ui_contract.get(key) is not False for key in ("staticGraphMutationFromUi", "targetRepositoryMutationFromUi", "physicsLayoutOnLoad")):
...
def validate_output(output: Path, project_workspace: Path, expected_projection: dict[str, Any], project_before: list[dict[str, str]]) -> dict[str, Any]:
    ...
    for marker in ["id=\"projectGraph\"", "id=\"modeBadge\"", "id=\"workList\"", "id=\"previousWork\"", "id=\"nextWork\"", "id=\"workPosition\"", "id=\"stageList\"", "id=\"previousStage\"", "id=\"nextStage\"", "id=\"stagePosition\"", "id=\"selectionInspector\"", "id=\"changePanel\"", "id=\"deltaList\"", "id=\"evidencePanel\"", "id=\"authorityPanel\"", "id=\"zoomIn\"", "id=\"zoomOut\"", "assets/cytoscape.min.js"]:
