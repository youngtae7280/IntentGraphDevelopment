...
P9.31 refines only that renderer. Cached node sprites now use a brushed
spectral-titanium treatment with asymmetric spectral rims, micro-etching, and a narrow
selected-state Fresnel ring. The precision node material core is bounded near 20 screen pixels,
while effective `100x`, the `0.065px`/`0.34` selected-edge taper, full graph contents,
layout coordinates, workflow, source state, and authority remain unchanged. The next
quality gap is repeatable browser-level rendering verification rather than another
manual visual observation.

P9.32 closes that quality gap with a read-only page probe and a dependency-free
headless Edge/Chrome runner. It checks the complete measured graph, effective `100x`,
actual endpoint geometry expansion, selected-edge screen scale, cached material identity
and pixels, edge inspection, runtime errors, and a nonblank PNG from the same process.
Input and browser digests bind the result, and eighteen negative probes prove the
validator and output guards fail closed. Graph, source, snapshot, target, layout,
...
P9.34.R3 applies the next user-requested presentation correction without changing graph
structure. Actual camera zoom reaches `512x` while retaining `100x` and `256x` jumps.
Maximum selected-relation emphasis is reduced to `0.08px`/`0.12`, and relation endpoints
use compact ticks instead of full selected-node halos. Cached
`nebula-black-metal-v7` sprites replace layered glossy rims with dark alloy facets,
etched contours, sparse grain, and one thin asymmetric diffraction rim. WindowsUtility
browser, negative, and loopback server gates pass over the unchanged 8,194-node /
7,986-relation projection.
```
