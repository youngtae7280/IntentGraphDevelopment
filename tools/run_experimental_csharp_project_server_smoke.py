...
def run(snapshot: Path, output: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="p9.15-server-smoke-") as temporary:
        ...
        try:
            ...
            probes.append({"id": "serves-512x-graphify-style-continuous-edges", "passed": all(marker in html for marker in (b"rendererMaximumZoom=512", b"effectiveGeometryZoom", b"virtualGeometryScale", b"selectedEdgeRenderedOpacity", b"selectedSourceRenderedWidth", b"selectedEdgeLinePixelEvidence", b"selectedEdgeLinePixels", b"communityPalette", b"hubLabelThreshold", b"normalizedEdgeConfidence", b"unsupportedConfidenceFallsBackToUnknown", b"edge[confidence = \"unknown\"]", b"selectedEdgeArrowShape", b"target-arrow-shape':'none'", b"graphifyPaletteApplied", b"allVisibleNodeShapesDot", b"materialOverlayPresent", b"zoomAnchorModelPosition", b"intentGraphRuntimeProbe", b"intentgraph-runtime-probe-report", b"graphPixelEvidence"))})
            ...
            probes.append({"id": "projects-512x-rendering-contract", "passed": initial_projection["uiContract"]["rendererMaximumZoom"] == 512 and initial_projection["uiContract"]["actualCameraMaximumZoom"] is True and initial_projection["uiContract"]["effectiveGeometryMaximumZoom"] == 512 and initial_projection["uiContract"]["virtualGeometryScaleAtMaximumZoom"] == 1.0 and initial_projection["uiContract"]["selectedEdgeRenderedWidthPixelsAtMaximumZoom"] == 2.0 and initial_projection["uiContract"]["selectedEdgeRenderedOpacityAtMaximumZoom"] == 0.90 and initial_projection["uiContract"]["overviewOrdinaryCodeOpacity"] == 1.0 and initial_projection["uiContract"]["overviewCodeRelationOpacity"] == 0.70 and initial_projection["uiContract"]["graphifyCommunityDotRendering"] is True and initial_projection["uiContract"]["graphifyCategoricalPalette"] is True and initial_projection["uiContract"]["degreeWeightedNodeSizing"] is True and initial_projection["uiContract"]["hubOnlyDefaultLabels"] is True and initial_projection["uiContract"]["confidenceStyledRelations"] is True and initial_projection["uiContract"]["singleRendererCoordinateSpace"] is True and initial_projection["uiContract"]["continuousEdgesDuringViewportInteraction"] is True and initial_projection["uiContract"]["cachedCanvasNodeMaterial"] is False and initial_projection["uiContract"]["selectedMaterialPixelEvidence"] is False and initial_projection["uiContract"]["virtualPrecisionZoom"] is False and initial_projection["uiContract"]["browserRuntimeProbe"] is True and initial_projection["uiContract"]["headlessBrowserRegression"] is True and initial_projection["uiContract"]["canvasPixelEvidence"] is True})
