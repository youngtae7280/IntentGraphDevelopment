...
def run(snapshot: Path, output: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="p9.15-server-smoke-") as temporary:
        ...
        try:
            ...
            probes.append({"id": "serves-actual-100x-and-cached-optical-material", "passed": all(marker in html for marker in (b"rendererMaximumZoom=100", b"effectiveGeometryZoom", b"virtualGeometryScale", b"selectedEdgeRenderedOpacity", b"culledOrdinaryCode", b"node-material-layer", b"materialSprite", b"materialCandidates", b"materialNodePixelEvidence", b"cached-astral-forged-glass-v3", b"intentGraphRuntimeProbe", b"intentgraph-runtime-probe-report", b"graphPixelEvidence", b"scheduleMaterialLayer"))})
            ...
            probes.append({"id": "projects-actual-100x-rendering-contract", "passed": initial_projection["uiContract"]["rendererMaximumZoom"] == 100 and initial_projection["uiContract"]["actualCameraMaximumZoom"] is True and initial_projection["uiContract"]["effectiveGeometryMaximumZoom"] == 100 and initial_projection["uiContract"]["virtualGeometryScaleAtMaximumZoom"] == 1.0 and initial_projection["uiContract"]["selectedEdgeRenderedWidthPixelsAtMaximumZoom"] == 0.55 and initial_projection["uiContract"]["selectedEdgeRenderedOpacityAtMaximumZoom"] == 0.68 and initial_projection["uiContract"]["astralForgedGlassNodeMaterial"] is True and initial_projection["uiContract"]["spectralTitaniumNodeMaterial"] is False and initial_projection["uiContract"]["virtualPrecisionZoom"] is False and initial_projection["uiContract"]["browserRuntimeProbe"] is True and initial_projection["uiContract"]["headlessBrowserRegression"] is True and initial_projection["uiContract"]["canvasPixelEvidence"] is True and initial_projection["uiContract"]["cachedCanvasNodeMaterial"] is True and initial_projection["uiContract"]["boundedMaterialSpriteCache"] is True and initial_projection["uiContract"]["viewportLocalMaterialRendering"] is True and initial_projection["uiContract"]["viewportSpatialMaterialIndex"] is True and initial_projection["uiContract"]["farZoomMaterialCulling"] is True and initial_projection["uiContract"]["selectedMaterialPixelEvidence"] is True})
