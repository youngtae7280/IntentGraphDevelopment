"""Run repeatable negative probes for the CF0 P1.14 usage/arity delta."""
...
GOOD_DELTA = ROOT / "docs/examples/cf0-python-cli-calculator/deltas/p1.14-overlay-usage-arity.delta.json"
BEFORE_FACTS = ROOT / "generated/cf0-python-cli-calculator/p1.14-before-code-facts.json"
BEFORE_OVERLAY = ROOT / "generated/cf0-python-cli-calculator/p1.14-before-overlay.json"
...
def replace_added_unit(delta: dict[str, Any], after_facts: dict[str, Any], overlay: dict[str, Any], tmp: Path) -> dict[str, Path]:
    delta["expectedAfter"]["addedBehaviorUnits"] = ["unit.behavior.usage-arity.missing"]
    return {}


def remove_usage_fact(delta: dict[str, Any], after_facts: dict[str, Any], overlay: dict[str, Any], tmp: Path) -> dict[str, Path]:
    after_facts["facts"] = [
        fact
        for fact in after_facts.get("facts", [])
        if fact.get("id") != "fact.function.main.stderr.usage_calc_add_sub_mul_left_right"
    ]
...
def remove_usage_verification(delta: dict[str, Any], after_facts: dict[str, Any], overlay: dict[str, Any], tmp: Path) -> dict[str, Path]:
    overlay["verification"] = [
        item for item in overlay.get("verification", []) if item.get("id") != "test.case.usage-arity"
    ]
...
def remove_expected_stderr(delta: dict[str, Any], after_facts: dict[str, Any], overlay: dict[str, Any], tmp: Path) -> dict[str, Path]:
    for item in overlay.get("verification", []):
        if item.get("id") == "test.case.usage-arity":
...
PROBES: list[dict[str, Any]] = [
    ...
    {
        "id": "missing-usage-arity-added-unit",
        "mutation": "Replace expected added behavior unit with a missing usage/arity unit id.",
        "expectedErrorSubstring": "missing added behavior units",
        "mutate": replace_added_unit,
    },
    {
        "id": "missing-usage-arity-code-fact",
        "mutation": "Remove fact.function.main.stderr.usage_calc_add_sub_mul_left_right from after facts.",
        "expectedErrorSubstring": "references missing fact fact.function.main.stderr.usage_calc_add_sub_mul_left_right",
        "mutate": remove_usage_fact,
    },
    {
        "id": "missing-usage-arity-verification",
        "mutation": "Remove test.case.usage-arity from after overlay verification records.",
        "expectedErrorSubstring": "missing expected behavior checks",
        "mutate": remove_usage_verification,
    },
    {
        "id": "missing-expected-stderr",
        "mutation": "Remove expectedStderr from usage/arity verification.",
        "expectedErrorSubstring": "stderr contract missing",
        "mutate": remove_expected_stderr,
    },
    {
        "id": "wrong-expected-stderr",
        "mutation": "Set usage/arity expectedStderr to the wrong value.",
        "expectedErrorSubstring": "stderr mismatch",
        "mutate": set_wrong_expected_stderr,
    },
    {
        "id": "wrong-expected-exit-code",
        "mutation": "Set usage/arity expectedExitCode to 0.",
        "expectedErrorSubstring": "exit code mismatch",
        "mutate": set_wrong_expected_exit_code,
    },
    {
        "id": "missing-p1.14-evidence-id",
        "mutation": "Replace P1.14 evidence ids with a missing id.",
        "expectedErrorSubstring": "missing delta evidence records",
        "mutate": set_missing_evidence_id,
    },
    {
        "id": "missing-p1.14-authority-id",
        "mutation": "Replace P1.14 authority ids with a missing id.",
        "expectedErrorSubstring": "missing delta authority records",
        "mutate": set_missing_authority_id,
    },
    {
        "id": "missing-p1.14-history-id",
        "mutation": "Replace P1.14 history ids with a missing id.",
...
def build_report() -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="cf0-usage-arity-negative-probes-") as tmp_name:
        return build_negative_probe_report(
            artifact_role="intentgraph-cf0-usage-arity-negative-probes-report",
            scope="cf0-p1.14-usage-arity-negative-probes",
            baseline_scope="current-p1.14-overlay-only-usage-arity-delta",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run CF0 P1.14 usage/arity negative probes.")
